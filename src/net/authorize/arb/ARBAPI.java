...
public class ARBAPI {
	 ...
	 * 
	 * current_request (ARBAPI)
	 */
	private BasicXmlDocument current_request = null;
	/**
	 * WRITEME: document this field (brpocock@star-hope.org, Nov 19, 2009)
	 * 
	 * current_response (ARBAPI)
	 */
	 ...
	 */
	public ARBAPI (final URL in_api_url, final String in_merchant_name,
	...
	private void addAuthenticationToRequest (
		...
		trans_key.appendChild (document.getDocument ().createTextNode (
				transaction_key));
	...
	private void addBillingInfoToSubscription (
			final BasicXmlDocument document,
			final ARBSubscription subscription, final Element subscr_el) {
		...
		if (bill_info.getZip () != null) {
			...
			zip_el.appendChild (document.getDocument ().createTextNode (
					bill_info.getZip ()));
	...
	private void addCustomerInfoToSubscription (
		...
		if (cust_info.getFaxNumber () != null) {
			...
			fax_el.appendChild (document.getDocument ().createTextNode (
					cust_info.getFaxNumber ()));
	...
	private void addPaymentScheduleToSubscription (
			final BasicXmlDocument document,
			final ARBSubscription subscription, final Element subscr_el) {
		final ARBPaymentSchedule schedule = subscription.getSchedule ();
		if (schedule == null)
			return;
		...
		if (schedule.getIntervalLength () > 0) {
			...
			unit_el.appendChild (document.getDocument ()
					.createTextNode (schedule.getSubscriptionUnit ()));
		...
		.appendChild (document
				.getDocument ()
				.createTextNode (
						net.authorize.arb.util.DateUtil
						.getFormattedDate (
								schedule
								.getStartDate (),
	...
	private void addPaymentToSubscription (
		...
		if (bank_account != null) {
			...
			ach_acc_type_el.appendChild (document.getDocument ()
					.createTextNode (bank_account.getAccountType ()));
			...
			ach_route_el.appendChild (document.getDocument ()
					.createTextNode (bank_account.getRoutingNumber ()));
			...
			ach_account_el.appendChild (document.getDocument ()
					.createTextNode (bank_account.getAccountNumber ()));
			...
			ach_name_el.appendChild (document.getDocument ()
					.createTextNode (bank_account.getNameOnAccount ()));
			ach_el.appendChild (ach_name_el);

			final Element ach_type_el = document
			.createElement (ARBAPIRequests.ELEMENT_ECHECK_TYPE);
			ach_type_el.appendChild (document.getDocument ()
					.createTextNode (bank_account.getEcheckType ()));
			...
			if (bank_account.getBankName () != null) {
				...
				bank_name_el.appendChild (document.getDocument ()
						.createTextNode (bank_account.getBankName ()));
	...
	private void addSubscriptionIdToRequest (
		...
		if (subscription.getSubscriptionId () != null) {
			...
			subscr_id_el
			.appendChild (document.getDocument ()
					.createTextNode (
	...
	private void importResponseMessages () {
		final NodeList messages_list = current_response.getDocument ()
		...
		result_subscription_id = getElementText (current_response
				.getDocumentElement (),
		...
		for (int i = 0; i < message_list.getLength (); i++ ) {
			final Element message_el = (Element) message_list.item (i);
	...
	public BasicXmlDocument sendRequest (
		...
		final String in_response = http_util.postUrl (request_document
				.dump ());
		...
		response.parseString (in_response.substring (mark, in_response
				.length ()));
		 ...
		 * FileOutputStream("out.xml",true);
		 * fos.write(in_response.getBytes()); } catch(Exception e){
