/**
 * <p>
 * Copyright © 2012, Bruce-Robert Pocock; Copyright © 2009-2010, Timothy W. Heys
 * </p>
 * <p>
 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
	...
	final class DisplayInterfaceRunner implements Runnable {
		...
		public void run () {
			Rahab.logger
					.info ("Building Application User Interface");
			applicationGUI = createAndShowGUI ();
			applicationGUI
					.showSystemMessage ("Application Initialized.");
			applicationGUI.showSystemMessage ("Connecting to "
					+ server);
		}
	}
	...
	public static void createAndShowHTMLPage (final String url) {
		...
		htmlInterface.setOpaque (true); // content panes must be
									// opaque
	...
	public static void createAndShowLoginPrompt () {
		...
		loginInterface.setOpaque (true); // content panes must be
									// opaque
	...
	public static void main (final String [] args) {
		 ...
		 * changed. Instead of using strong references, it now uses
		 * weak references internally. That's a reasonable change, but
		 ...
		 * 									// configuration
		 * 	logger.setUseParentHandlers (false); // another call to change
		 ...
		 * logger configuration is lost (because Logger only keeps
		 * weak references).
		 * </p>
		 * 
		 * <pre>
		...
		public void handleEvent (final SmartFauxEvent event) {
			try {
				...
				PubliusVergiliusMaro
						.reportBug (
								"Caught a JSONException in .handleEvent ",
								e);
			}
		}
	};
	...
	private final SmartFauxEventListener onExtensionRequest = new SmartFauxEventListener () {
		@Override
		public void handleEvent (final SmartFauxEvent event) {
			Rahab.logger.info ("Received an xt request: "
					+ event.toString ());
			try {
				...
				if (dataObj.has ("_cmd")) {
					...
					applicationGUI.showDialog (
							dataObj.getString ("title"),
							dataObj.getString ("label"),
	...
	private final SmartFauxEventListener onRoomListUpdate = new SmartFauxEventListener () {
		...
		@Override
		public void handleEvent (final SmartFauxEvent event) {
			sfClient.joinRoom (Rahab.$EAVES, Rahab.roomPassword,
					false);
		}
	};
	...
	private final SmartFauxEventListener onUserEnterRoom = new SmartFauxEventListener () {
		...
		public void handleEvent (final SmartFauxEvent event) {
			...
			try {
				userThatLeft = event.getParams ().getString (
						"userName");
			} catch (final JSONException e) {
				PubliusVergiliusMaro
						.reportBug (
	...
	protected SpyUI createAndShowGUI () {
		...
		applicationGUI.setOpaque (true); // content panes must be
									// opaque
	...
	protected void disectMessageEvent (final String messageEventParam) {
		if (messageEventParam.contains (Rahab.unitSep)) {
			...
			try {
				...
				Rahab.logger.info ("Unhandled message: "
						+ messageEventParam);
			}
	...
	private void doMessageEventAction (final String user,
			final String room, final Actions verb,
		...
		case part:
			...
			break;
		case ban:
			applicationGUI.showSaidMessage (user, room,
					"was banned " + message);
			break;
		case kick:
			applicationGUI.showSaidMessage (user, room,
		...
		case Oops:
			applicationGUI
					.showBlackListMessage (user, room, message);
			break;
		case REDLIST:
	...
	protected void extractRoomListFromJSON (final JSONObject dataObj)
			throws JSONException {
		final JSONObject roomListJSON = dataObj
				.getJSONObject ("rooms");
		final Iterator <String> keys = roomListJSON.keys ();
		while (keys.hasNext ()) {
			final String room = keys.next ();
			final Iterator <String> users = roomListJSON
					.getJSONObject (room).keys ();
			while (users.hasNext ()) {
				final String userInRoom = roomListJSON
	...
	protected Vector <Zone> getZoneListFromJSON (
		...
		while (keys.hasNext ()) {
			...
			final JSONObject zoneObj = zoneList
			...
			final int usersOn = Integer.parseInt (zoneObj
					.getString ("usersOn"));
			Rahab.logger.fine ("New zone (#" + keyName + "):  "
					+ zoneName + "\n" + zoneHost + "\n" + usersOn);
	...
	private void initSFListeners () {
		...
		sfClient.addEventListener (SmartFauxEvent.onConnection,
				onConnection);
		sfClient.addEventListener (SmartFauxEvent.onConnectionLost,
				onConnectionLost);
		sfClient.addEventListener (SmartFauxEvent.onRandomKey,
				onRandomKey);
		sfClient.addEventListener (SmartFauxEvent.onRoomListUpdate,
				onRoomListUpdate);
		sfClient.addEventListener (
				SmartFauxEvent.onExtensionResponse,
				onExtensionRequest);
		sfClient.addEventListener (SmartFauxEvent.onJoinRoom,
				onJoinRoom);
	...
	protected void loginToZone (final String zone) {
		if (null == zone) {
			Rahab.logger
					.info ("Failed to join zone. Reason: Zone is null.");
			return;
		}
		currentZone = zone;
		...
		sfClient.logout ();
		sfClient.login (zone, userName,
				Rahab.getApple (key, password));
	}
