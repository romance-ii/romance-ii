...
public class SpyUI extends JPanel {
    ...
    */
	...
	public SpyUI (final MessageCallBack actionCallBack,
		...
		zoneChatStyledDocument = zoneChatTextPane
				.getStyledDocument ();
		gameMailTextPane = new JTextPane ();
		gameMailTextPane.setEditable (false);
		gameMailStyledDocument = zoneChatTextPane
				.getStyledDocument ();
	...
	private void addLeafToTree (final String leafName,
		...
		try {
			roomBranch = getBranchByNameFromRoot (branchName,
					rootTree);
		} catch (final Exception e) {
	...
	protected void addStylesToDocument (final StyledDocument document) {
		...
		final Style displayText = document.addStyle (
				SpyUI.NORMAL_TEXT_STYLE, defaultStyle);
		...
		// Create and add the style for displaying text
		final Style grayText = document.addStyle (
				SpyUI.GRAY_TEXT_STYLE, defaultStyle);
	 ...
	 * @param room WRITEME
	 */
	public void addUserToHouse (final String userName,
			final String room) {
	...
	private JTabbedPane getTabbedPane () {
		final JScrollPane zoneChatTextPaneScroll = SpyUI
				.getScrollForTextPane (zoneChatTextPane);
		addStylesToDocument (zoneChatStyledDocument);
		final JScrollPane gameMailTextPaneScroll = SpyUI
				.getScrollForTextPane (gameMailTextPane);
		...
		try {
			zoneChatStyledDocument.insertString (
					zoneChatStyledDocument.getLength (),
					getLogo (), zoneChatStyledDocument
	...
	private JScrollPane getUserListTab () {
		...
		final JScrollPane userJListScroll = new JScrollPane (
				userList,
				ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
	...
	public void initListOfUsers (
			final Vector <String> newUserListVector) {
	...
	private void scrollZoneOutputToBottom () {
		if (zoneChatTextPane.getAutoscrolls ()) {
			zoneChatTextPane
					.setCaretPosition (zoneChatStyledDocument
							.getLength ());
		}
	}
	...
	public void showBlackListMessage (final String speaker,
		...
		showMessage (speaker, room, "filtered{" + message.trim ()
	...
	public void showDialog (final String title, final String label,
		...
		final String dialog = String.format ("%s%n%s %s", title,
				label, message);
		...
		try {
			...
			zoneChatStyledDocument.insertString (
					...
					userNameDisplayText, zoneChatStyledDocument
							.getStyle (userNameStyle));
			zoneChatStyledDocument.insertString (
					zoneChatStyledDocument.getLength (), message,
					zoneChatStyledDocument
							.getStyle (SpyUI.ERROR_TEXT_STYLE));
		} catch (final BadLocationException e) {
			e.printStackTrace ();
	...
	private void showMessage (final String speaker, final String room,
		...
		if (SpyUI.SYSTEM_SPEAKER.equals (speaker)) {
			userNameDisplayText = String.format ("(%s)%s ",
					timeStamp, speaker);
		}
		...
		try {
			...
			zoneChatStyledDocument.insertString (
					zoneChatStyledDocument.getLength (),
					userNameDisplayText, userNameStyle);
			zoneChatStyledDocument.insertString (
					zoneChatStyledDocument.getLength (), message,
					displayTextStyle);
		} catch (final BadLocationException e) {
	...
	public void showModMessage (final String speaker,
		...
		showMessage (speaker, "SPY", message, userNameStyle,
				textStyle);
	}
	...
	public void showReport (final String reportingUser,
		...
		try {
			...
			zoneChatStyledDocument.insertString (
					zoneChatStyledDocument.getLength (),
					reportMessage, zoneChatStyledDocument
							.getStyle (SpyUI.ERROR_TEXT_STYLE));
		} catch (final BadLocationException e) {
			e.printStackTrace ();
		}
	...
	public void showSystemMessage (final String systemMessage) {
		...
		showMessage (SpyUI.SYSTEM_SPEAKER, "", systemMessage,
				userNameStyle, textStyle);
	}
}
