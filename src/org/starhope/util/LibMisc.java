...
public class LibMisc {
	...
	public static boolean areWeThereYet (final AbstractUser thing,
			final long when) {
		return Geometry.getTimeToTarget (thing, when) > 0 ? false
				: true;
	}
	...
	public static void commandJSON (final String cmd,
			...
			final AbstractUser user, String channel,
			final Class <?> klass) {
		final Class <?> extension = LibMisc.loadExtension (klass);

		Method commandProcessor = null;
		try {
			commandProcessor = klass
					.getMethod ("do_" + cmd, JSONObject.class,
		...
		if (null != commandProcessor) {
			...
			if (currentThread instanceof ServerThread) {
				if ( ((NetIOThread) currentThread).isParallelMode ()) {
					final CommandExecutorThread t = new CommandExecutorThread (
							cmd, jso, commandThread, user, channel,
							commandProcessor);
					((ServerThread) currentThread).adopt (t);

				} else {
					LibMisc.executeCommand (cmd, jso, commandThread,
							user, channel, commandProcessor);
				}
			}
		}
	}
	 ...
	 * Prefer using {@link Coord2D#distance(Coord2D)} for most purposes.
	 * 
	 ...
	 * JSONObject, AbstractUser, and Room. The reflective command Method
	...
	public static void executeCommand (final String cmd,
		...
		try {
			final String channelString = channel != null ? channel
					...
					.getRoom () == null ? "" : user
					.getRoom ().getMoniker ();
			final Channel ch = null == user ? null : user
					.getZone () != null? user.getZone ().getChannel (
					channelString):null;
			...
			LibMisc.sendJSONCommandBugReport (cmd, jso, commandThread,
					user, e);
	...
	public static String formatFutureDate (final Date targetDate,
		...
		if ( !language_dialect.equals ("en_US")) {
			AppiusClaudiusCaecus
					.reportBug ("Formatting future date in English for user who wanted "
							+ language_dialect);
		}
	...
	private static String formatFutureDate_English (
		...
		if (days > 600) {
			return String.format ("%s years from now", LibMisc
					.cardinalNumber_English ((int) (days / 365.2489)));
		} else if (days > 31) {
	...
	public static String formatMemory (final long numBytes) {
		...
		if (numBytes > 1024 * 10240) {
			return String.format ("%.1f MiB",
					Double.valueOf ((double) numBytes / (1024 * 1024)));
		}
		if (numBytes > 10240) {
	...
	private static String formatPastDate_English (final Date targetDate) {
		...
		if (diffSec < 60) {
			return String.format ("%s seconds ago",
		...
		if (diffSec < 120) {
			...
			return String.format ("a minute and %s seconds ago",
					LibMisc.cardinalNumber_English ((int) diffSec));
		}

		if (diffSec < 60 * 90) {
			return String.format ("%s minutes ago", LibMisc
					.cardinalNumber_English ((int) (diffSec / 60)));
		}
		...
		final int days = (int) (diffSec / (60 * 60 * 24));
		if (days > 600) {
			return String.format ("%s years ago", LibMisc
					.cardinalNumber_English ((int) (days / 365.2489)));
		} else if (days > 31) {
	 ...
	 *            catalogue.
	 * @param fallback The fallback text to be used if that message does
	 *            not exist.
	 ...
	 * @param fallback An expression to be returned if the specified key
	 *            is not found in the message catalogue
	...
	/**
	 * Determine whether a translation for a given message key exists in
	 * the catalogue for a certain language/dialect.
	 *
	 ...
	 * org.starhope.messages.LANG_DIALECT, and must be enumerated in the
	 * “master” list of org.starhope.messages.
	 * </p>
	 * <p>
	 * In the absence of either key, the default message language set is
	 * simply en_US (US English), and the default message catalogues are
	 * /etc/appius/messages/$LANG_DIALECT.properties, e.g.
			...
			try {
				...
				filer = new FileReader (
						AppiusConfig.getConfigOrDefault (
								"org.starhope.messages." + lang,
								"/etc/appius/messages/" + lang
										+ ".properties"));
				catalog.load (filer);
	 ...
	 * map), perform a fairly sketchy randomization upon it which should
	 * be “good enough” for casual (e.g. non-security-related) purposes.
	 * </p>
	 *
	...
	private static void stringify_column (final ResultSet rs,
		...
		case Types.DECIMAL:
			...
			break;
		default:
			s.append (String.format ("$(%08x)", Integer.valueOf (type)));
		}
	}
	...
	/**
	 * convert a user-visible string into a javaCased moniker; takes out
	 * all non-alpha chars and uses them as upper-case indicators
	 * 
	...
	public static String toJavaCase (final String source) {
		...
		String working = source.toLowerCase (Locale.ENGLISH);
	 ...
	 * @return a new JSON object with elements numbered ascending from 0
	 */
	public static <T extends Object> JSONObject toJSON (
			final Collection <T> collection,
			final boolean onlyString) {
	 ...
	 * (including carriage returns, newlines, and horizontal or vertical
	 * tabulations) with a single space, and trims off the leading and
	 * trailing whitespace.
	 * </p>
	 * <p>
