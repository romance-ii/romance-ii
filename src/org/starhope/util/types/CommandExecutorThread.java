...
public class CommandExecutorThread extends Thread implements
	...
	public static StringBuilder dumpTimes () {
		...
		for (final Entry <String, Queue <Long>> cmd : CommandExecutorThread.profiles
				.entrySet ()) {
			times.append (cmd.getKey ());
			final Queue <Long> entries = cmd.getValue ();
			long sum = 0;
			long least = Long.MAX_VALUE;
			long greatest = 0;
			for (final Long val : entries) {
				final long i = val.longValue ();
				if (i > greatest) {
					greatest = i;
				}
				if (i < least) {
	...
	public void run () {
		...
		LibMisc.executeCommand (cmd, jso, commandThread, user,
