...
public class GaiusValeriusCatullus extends UnicastRemoteObject
	...
	static final class LocalServerSocketFactory implements
		...
		/**
		 * create a {@link LocalServerSocketFactory} for the given host
		...
		public ServerSocket createServerSocket (final int localPort)
				throws IOException {
			return new ServerSocket (localPort, 10, InetAddress
					.getByName (host));
		}
	}
	...
	static <T extends DataRecord> T setAll (final T dest, final T source)
		...
		if (Copyable.class.isInstance (dest)) {
			...
			GaiusValeriusCatullus.setAll_annotatedClass (dest, source,
					klass);
		}

		if (null == oldLoader || oldLoader instanceof NullLoader <?>) {
			...
			if (m.isAnnotationPresent (Setter.class)) {
				...
				if (parameterTypes.length < 1) {
					AppiusClaudiusCaecus
							.reportBug ("Setter taking no parameters found: only single-object setters are supported by Catullus: "
				...
				if (parameterTypes.length > 1) {
					AppiusClaudiusCaecus
							.reportBug ("Setter taking multiple parameters found: only single-object setters are supported by Catullus: "
									+ klass.getCanonicalName ()
									+ "::"
									+ m.toGenericString ());
					continue METHODS;
				...
				GaiusValeriusCatullus.setAll_setFromGetter (dest,
						source, klass, m, getName, parameterTypes,
						kind, getter);
			}
		}
		if ( !seenOne) {
	...
	public GaiusValeriusCatullus (final String newServiceName)
		...
		AppiusClaudiusCaecus
				.blather ("Attaching C. Valerius Catullus server to "
						+ serviceURL.toString ());
	...
	private <T extends DataRecord, S extends T> T getLocalPeer (
		...
		try {
			...
			try {
				return Nomenclator.getDataRecord ((Class <S>) obj
						.getClass (), ident);
			...
			try {
				return Nomenclator.getDataRecord ((Class <S>) obj
						.getClass (), obj.getCacheableID ());
			} catch (final NotFoundException e1) {
				// no op
			}
		}
		try {
			return GaiusValeriusCatullus.setAll ( ((Class <S>) obj
	...
	public <T extends DataRecord> T read (final Class <T> klass,
		...
		AppiusClaudiusCaecus.blather ("read:"
				+ klass.getCanonicalName () + "/#" + id);
	...
	public <T extends DataRecord> void remove (final T obj) {
		final RecordLoader <T> recordLoader = (RecordLoader <T>) getLocalPeer (
				obj).getRecordLoader ();
		AppiusClaudiusCaecus.blather ("remove:" + obj.toString ());
	...
	public <T extends DataRecord> T write (final T obj) {
		AppiusClaudiusCaecus.blather ("write:" + obj.toString ());
