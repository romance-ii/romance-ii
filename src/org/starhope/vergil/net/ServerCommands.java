...
public abstract class ServerCommands {
	...
	public static void acceptServerResponse (final JSONObject jso)
		...
		final boolean success = jso.getBoolean ("status");
		final String source = jso.getString ("from");
		if (success) {
			ServerCommands.dispatchSuccess (source, jso);
		} else {
			ServerCommands.dispatchError (source,
					jso.getString ("err"), jso.optString ("msg"),
					jso);
		}
	}
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_deleteMailMessage_err (
			final JSONObject jso);
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_deleteMailMessage_ok (
			final JSONObject jso);
	 ...
	 * although Tootsville had been using a “thought bubble.” The
	 * visual display is irrelevant to the server.
	 * </p>
	 * 
	 * @param jso One of the following formats: { from: earnings,
	 *             currency: { CURRENCY-OBJECT }, amount: NUMBER }, or,
	 *             { from: earnings, item: ITEM-ID-NUMBER }, or, {
	 *             from: earnings, stat: STAT-NAME, change:
	 *             CHANGE-AMOUNT }
	 */
	 ...
	 * Equipment is defined as items which are active (equipped,
	 * donned), which have either an ACTIVE or PASSIVE effect. This is
	 * sent as an unordered list (using a JSON array-type list) of all
	 * items which meet these criteria.
	 * </p>
	 * <p>
	 * Note that receiving this message could mean either an item has
	 * been equipped or de-equipped, or that an item has changed; e.g.
	 * a change to its health or colour.
	 * </p>
	 * 
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_getAvailableHouses_err (
			final JSONObject jso);
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_getAvailableHouses_ok (
			final JSONObject jso);
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_getAwardRankings_err (
			final JSONObject jso);
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_getColorPalettes_err (
			final JSONObject jso);
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_getInventoryItems_err (
			final JSONObject jso);
	 ...
	 * @param jso JSON object from the server WRITEME
	 */
	public abstract void from_getInventoryItems_ok (
			final JSONObject jso);
	 ...
	 * WRITEME
	 * 
	 * @param jso from: getWallet, walletOwner: YOURNAME, currency: {
	 *             "x-TvPn": 500.00 }
	 */
	 ...
	 * </p>
	 * <p>
	 * <b> Change in 1.1: </b> the userName field was previously
	 * labeled as “user”
	 * </p>
	 * 
	 * @param jso <dl>
	 ...
	 * 
	 * @param jso { sound: SOUND-IDENTIFIER-STRING } or { url:
	 *             SOUND-URL }
	 */
	...
	/**
	 * @see Commands#do_savePrefs(JSONObject,
	 *      org.starhope.appius.user.AbstractUser,
	 *      org.starhope.appius.game.Room)
	 * @param jso JSON object from the server WRITEME
	 */
