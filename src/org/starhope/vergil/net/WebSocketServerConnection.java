...
public class WebSocketServerConnection implements ServerConnection {
	...
	public void connect (final String newHost, final int newPort,
			final String newZone) throws IOException,
			ServerDisconnectedException {
		host = newHost;
		port = newPort;
		zone = newZone;
		socket = new WebSocket (newHost, newPort, "/" + newZone);
	}
	...
	/**
	 * @see org.starhope.vergil.net.ServerConnection#readMessage()
	...
	public JSONObject readMessage ()
		...
		try {
			return JSONParser.parse (socket.getMessage ())
					.isObject ();
		} catch (final IOException e) {
