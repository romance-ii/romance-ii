...
public class EventPlanner {
	 ...
	 * @throws EventNotPlannedException WRITEME
	 */
	public static void criticalEvent (final String ident, final Object... data)
			throws EventNotPlannedException {
		final List <VergilEventHandler> list = EventPlanner.handlers.get (ident);
		if (null == list || 0 == list.size ()) {
			throw new EventNotPlannedException ();
		}
		boolean handled = false;
		for (VergilEventHandler handler : list) {
			boolean failed = false;
			try {
				handler.acceptEvent (VergilEventFactory
						.interpret (ident, data));
	 ...
	 */
	public static void ignore (final String ident, final VergilEventHandler handler) {
		List <VergilEventHandler> list = EventPlanner.handlers.get (ident);
		if (null == list) {
			return;
		}
		list.remove (handler);
	 ...
	 * @param handler WRITEME
	 */
	public static void listen (final String ident, final VergilEventHandler handler) {
