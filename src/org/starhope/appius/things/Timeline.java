...
public class Timeline {
    ...
    public List<Motion> getMotionForInterval (final long startTime, final long endTime) {
        final LinkedList<Motion> returns = new LinkedList<Motion>();
        for (final Motion motion : motions) {
            if (motion.getStartTime () >= startTime && motion.getStartTime () <= endTime) {
                returns.add(motion);
            } else if (motion.getStartTime() <= startTime && motion.getEndTime() >= endTime) {
                returns.add(motion);
            } else if (motion.getEndTime() >= startTime && motion.getEndTime() <= endTime) {
                returns.add(motion);
            }
        }
        return returns;
