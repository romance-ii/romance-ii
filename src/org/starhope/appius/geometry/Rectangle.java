...
public class Rectangle implements PolygonPrimitive <Rectangle> {
	 ...
	 */
    public Rectangle (final Coord2D oneCorner, final Coord2D otherCorner)
    {
        origin =
            new Coord2D (Math.min (oneCorner.x, otherCorner.x),
    ...
    public boolean contains (final double x, final double y) {
        return x >= origin.x && x <= origin.x + width && y >= origin.y
        && y <= origin.y + height;
    }
	...
	public Rectangle scale (final double sizeScalar) {
		return new Rectangle (origin.multiply (sizeScalar), width*sizeScalar, height*sizeScalar);
	}
