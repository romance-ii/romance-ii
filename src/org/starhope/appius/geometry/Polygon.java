...
public class Polygon implements PolygonPrimitive <Polygon> {
	...
	/**
	 * Constructs a closed polygon using the given set of points as line
	 * segments
	 * 
	 ...
	 * @throws ParameterException The number of x and y coordinates must
	 *             match
	 */
	public Polygon (final double [] xPoints, final double [] yPoints)
	throws ParameterException {
		if (xPoints.length != yPoints.length) throw new ParameterException (
	...
	private LineSeg2D [] computeSegments () {
		...
		if (points.length > 1) {
			...
			for (int i = 0; i < segs.length - 1; i++ ) {
				segs [i] = new LineSeg2D (points [i], points [i + 1]);
			}
	...
	/**
	 * Tests to see if we are inside of the polygon. Will give incorrect
	 * results if the polygon exceeds 100 000 units in size
	 * 
	...
	public boolean contains (final Coord2D coord2d) {
		// Create an arbitrary large line segment from the point to the
		// right.
		if (boundingCircle.contains (coord2d)) {
			final LineSeg2D ray = new LineSeg2D (coord2d, new Coord2D (
					100000 + coord2d.x, coord2d.y));
			int count = 0;
	...
	public Coord2D [] intersection (final LineSeg2D lSeg2d) {
		...
		for (final LineSeg2D segment : segments) {
			final Coord2D intPoint = segment.intersectionPoint (lSeg2d);
			if (null != intPoint) {
				set.add (intPoint);
			}
		}
	...
	public boolean intersects (final PolygonPrimitive <?> polygon) {
		...
		else if (polygon instanceof Rectangle) return intersectsRect ((Rectangle) polygon);
		else return intersectsPoly ((Polygon) polygon);
	}
	...
	public boolean intersectsCircle (final Circle circle) {
		...
		for (final LineSeg2D segment : segments) {
			if (circle.intersects (segment)) return true;
		}
		return false;
	}
		...
		if (boundingCircle.intersects (polygon.getBoundingCircle ())) {
			for (final LineSeg2D segment : polygon.segments) {
				for (final LineSeg2D segment2 : segments) {
					if (segment2.intersects (segment)) return true;
				}
			}
		}
		return false;
	...
	public Polygon translate (final double x, final double y) {
		...
		return new Polygon (newCoord2ds, newSegments, boundingCircle
				.translate (x, y));
	}
