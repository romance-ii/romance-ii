 ...
 * </p>
 * <p>
 * Copyright © 2011-2012, Bruce-Robert Pocock
 * </p>
 * <p>
...
public class Circle implements PolygonPrimitive <Circle> {
	...
	private double getB (final Coord2D point1, final Coord2D point2,
			final Coord2D cntr) {
		return ( ( (point2.x - point1.x) * (point1.x - cntr.x)) + ( (point2.y - point1.y) * (point1.y - cntr.y))) * 2;
	}
	...
	private double getC (final Coord2D point1, final Coord2D cntr,
		...
		return ( (cntr.x * cntr.x) + (cntr.y * cntr.y)
				+ (point1.x * point1.x) + (cntr.y * cntr.y))
				- (2 * ( (cntr.x * point1.x) + (cntr.y * point1.y)))
				- (radius0 * radius0);
	}
	...
	public Coord2D [] intersection (final LineSeg2D lsSeg2d) {
		...
		if (inside > 0) {
			...
			return new Coord2D [] {
					getIntPoint (lsSeg2d.point1, lsSeg2d.point2,
							u1),
					getIntPoint (lsSeg2d.point1, lsSeg2d.point2,
							u2) };
		} else if (inside == 0) {
	...
	/**
	 * Determines if this polygon intersects with another one XXX:
