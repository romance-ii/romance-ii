...
public class LineSeg2D {
	...
	public Coord2D intersectionPoint (final LineSeg2D lSeg2d) {
		...
		return (ua > 0) && (ua < 1) && (ub > 0) && (ub < 1) ? new Coord2D (
				point1.x + (ua * (point2.x - point1.x)), point1.y
						+ (ua * (point2.y - point1.y))) : null;
	}
	 ...
	 * Document this method ewinkelman@resinteractive.com
	 * 
	 * @param x WRITEME
	 * @param y WRITEME
	 * @return WRITEME
	...
	public LineSeg2D translate (final double x, final double y) {
		return new LineSeg2D (point1.translate (x, y),
