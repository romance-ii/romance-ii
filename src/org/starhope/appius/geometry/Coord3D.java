...
public class Coord3D extends Tuple3D <Coord3D> {
	...
	/**
	 * @param string A coordinate string as one of: JSON { x:###, y:###,
	 *        z:### } or (x,y,z) or [x:y:z]
	 * @return WRITEME
	 */
	 ...
	 */
	public static double inverseSquare (final Coord3D a, final Coord3D b)
	{
	...
	public static Coord3D randomIn (final Room aRoom) {
		...
		for (; !aRoom.canWalk (coord, coord); coord = new Coord3D (
				AppiusConfig.getRandomInt (minX, maxX), AppiusConfig
				.getRandomInt (minY, maxY), AppiusConfig
				.getRandomInt (minZ, maxZ))) {
			// No op
		}
		return coord;
	}
	...
	/**
	 * @param other another point
	 * @return the dot-product (scalar product) of the vectors from origin to the given points
	 */
	...
	public Coord2D toCoord2D () {
		if (Math.abs (z) > 0.00001d) {
			AppiusClaudiusCaecus
					.reportBug ("Flattening a non-planar 3D coordinate : "
							+ toString ());
		}
