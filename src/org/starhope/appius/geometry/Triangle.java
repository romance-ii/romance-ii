...
public class Triangle {
	...
	private double [] computeIntervals (final double VV0,
		...
		if (D0D1 > 0.0f) {
			...
			/*
			 * that is D0, D1 are on the same side, D2 on the other or
			 * on the plane
			 */
	...
	private boolean contains (final Coord3D point) {
		...
		AppiusClaudiusCaecus
				.reportBug ("unimplemented Triangle::contains (brpocock@star-hope.org, Oct 9, 2010)");
		return false;
	}

	/**
	 * Determine whether an entire triangle is contained within this one
	 *
	 ...
	 */
	private boolean coplanar_tri_tri (final Coord3D n1, final Triangle t) {
		short i0, i1;
		/*
		 * first project onto an axis-aligned plane, that maximizes the
		 * area of the triangles, compute indices: i0,i1.
		 */
		...
		return EDGE_AGAINST_TRI_EDGES (points [0], points [1],
				...
				|| EDGE_AGAINST_TRI_EDGES (points [1], points [2],
						t.points [0], t.points [1], t.points [2], i0,
						i1)
				|| EDGE_AGAINST_TRI_EDGES (points [2], points [0],
						t.points [0], t.points [1], t.points [2], i0,
						i1) ||
				/*
				 * finally, test if tri1 is totally contained in tri2 or
				 * vice versa
				 */
	...
	public boolean hasEdge (final LineSeg3D edge) {
		return (edge.hasEndpoint (points [0]) ? 1 : 0)
				+ (edge.hasEndpoint (points [1]) ? 1 : 0)
				+ (edge.hasEndpoint (points [2]) ? 1 : 0) > 1;
	}
	...
	public boolean hasVertex (final Coord3D vertex) {
		...
		return vertex.equals (points [0]) || vertex.equals (points [1])
	...
	public boolean intersects (final Triangle t) {
		...
		case 2:
			...
			break;
		default:
			throw AppiusClaudiusCaecus.fatalBug ("Unreachable");
		}
	...
	public Collection <? extends Triangle> subtract (final Triangle t) {
		// TODO Auto-generated method stub brpocock@star-hope.org
		AppiusClaudiusCaecus
				.reportBug ("unimplemented Triangle::subtract (brpocock@star-hope.org, Oct 9, 2010)");
		return null;
	}
