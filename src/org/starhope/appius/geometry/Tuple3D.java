...
CastsToJSON {
	...
	public Tuple3D (final double x0, final double y0, final double z0) {
		if (Double.isNaN (x0) || Double.isInfinite (x0)
			...
			throw new IllegalArgumentException ("finite coördinates are mandatory");
		}
		x = x0;
		y = y0;
		z = z0;
	...
	/**
	 * <p>Divides the tuple by the value </p>
	 * <p><strong>Note:</strong> Will happily try to divide
	 * by zero if assertions are disabled! </p>
	 *
	...
	/**
	 * Implementation: Takes the 64-bit binary representation of each of
	 * the double-precision floating-point values in this object,
	...
	@Override
	public int hashCode () {
		long blah = Double.doubleToRawLongBits (x) << 32
				^ Double.doubleToRawLongBits (y) << 16
				^ Double.doubleToRawLongBits (z);
		return (int) (blah & 0xffffffff)
				^ (int) (blah >> 32 & 0xffffffff);
	}
	 ...
	 */
	public T subtract (final double x2, final double y2, final double z2) {
