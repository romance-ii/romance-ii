...
public abstract class NetIOThread extends Thread implements
		...
		if (null != socket) {
			synchronized (socket) {
				try {
					...
					/*
					 * Ignore. Seems to mostly mean that the socket
					 * were closed anyways.
					 */
				}
				if ( !socket.isClosed ()) {
	...
	protected synchronized void dropSocketConnection () {
		...
		if ( (this != Thread.currentThread ())
			...
			BugReporter
					.getReporter ("srv")
					.reportBug (
							"DAMNED: The damned are to be judged only by those appointed by the gods, but "
	...
	/**
	 * @return whether to include backtraces in exception reports to
	 *         the client
	 */
	 ...
	 * @return An output packet for the pipe, or a zero-length string
	 *         if there is no input; or, a null pointer if the pipe
	 ...
	 * 
	 * @param really whether to enable this mode, or actually to
	 *             disable it.
	 */
