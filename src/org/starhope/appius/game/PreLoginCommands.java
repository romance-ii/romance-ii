...
public class PreLoginCommands {
	...
	public static JSONObject do_batch (final JSONObject jso,
		...
		final ServerThread myThread = PreLoginCommands
				.findMyself (result);
		if ( (null == myThread)
				|| ! (myThread instanceof BatchProcessor)) {
			return result;
		}
	 ...
	 *             only parameter interpreted is “id,” which must be
	 *             the user ID number whose public information is to be
	 *             returned.
	 * @param u unused
	...
	public static JSONObject do_finger (final JSONObject jso,
		...
		ok.put ("from", "finger");
		ok.put ("info", userByID.getDatagram (u).toString ());
		return ok;
	}
	...
	public static JSONObject do_login (final JSONObject jso,
		...
		try {
			Zone loginZone = null;
			try {
				loginZone = AppiusClaudiusCaecus.getZone (jso
						.getString ("zone"));
			} catch (final NotFoundException e) {
				// nada
			}
			if ( (null == loginZone)
					|| !myself.logIn (loginZone,
