...
public class RoomSQLLoader implements RecordLoader <Room> {
	...
	static Collection <Room> getRoomsInDB () {
		...
		try {
			...
			if (st.execute ()) {
				...
				while (rs.next ()) {
					try {
						everywhere.add (Nomenclator.getDataRecord (Room.class, rs
								.getString ("moniker")));
					} catch (final NotFoundException e) {
						AppiusClaudiusCaecus
								.reportBug (
										"Caught a NotFoundException in Room.getRoomsInDB ",
										e);
					}
				}
			}
	...
	public Room loadRecord (final int id) throws NotFoundException {
		...
		try {
			...
			throw AppiusClaudiusCaecus.fatalBug ("room load hatred", e);
		} finally {
			LibMisc.closeAll (roomVars, roomRS, getRoomVars, getRoom,
					con);
		}
	}
	...
	public void removeRecord (final Room record) {
		...
		AppiusClaudiusCaecus
		.reportBug ("unimplemented RoomSQLLoader::removeRecord (brpocock@star-hope.org, Aug 6, 2010)");
