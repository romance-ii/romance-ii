 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
 */
public class EventOutcome extends
		SimpleDataRecord <EventOutcome> {

	/**
	 * Logging
	 */
	private static final Logger log = LoggerFactory.getLogger (EventOutcome.class);
	...
	/**
	 * The array of possible item rewards and related information
	 */
	private WeakReference <OutcomeItems> outcomeItems = new WeakReference <OutcomeItems> (null);

	/**
	 * The array of possible item rewards and related information
	 */
	private WeakReference <OutcomeCurrencies> outcomeCurrencies = new WeakReference <OutcomeCurrencies> (null);
	 ...
	 */
	public EventOutcome (
			final RecordLoader <EventOutcome> loader) {
		super (loader);
	}
	...
	public boolean equals (final Object obj) {
		...
		if ( ! (obj instanceof EventOutcome)) {
			return false;
		}
	...
	/**
	 * Gets the outcome items for this outcome
	 *
	 * @return WRITEME 
	 */
	private OutcomeCurrencies getOutcomeCurrencies() {
		OutcomeCurrencies result = outcomeCurrencies.get ();
		if(result==null) {
			try {
				outcomeCurrencies =
						new WeakReference <OutcomeCurrencies> (Nomenclator.getDataRecord (OutcomeCurrencies.class, id));
				result = outcomeCurrencies.get ();
			} catch (NotFoundException e) {
				EventOutcome.log.error ("Exception", e);
			}
		}
		return result;
	}
	 ...
	 * @return WRITEME 
	 */
	private OutcomeItems getOutcomeItems() {
		OutcomeItems result = outcomeItems.get ();
		if(result==null) {
			try {
				outcomeItems = new WeakReference <OutcomeItems> (Nomenclator.getDataRecord (OutcomeItems.class, id));
				result = outcomeItems.get ();
			} catch (NotFoundException e) {
				EventOutcome.log.error ("Exception", e);
			}
		}
	...
	public Map <Currency, Long> getRewardCurrencies (final long points) {
		return getOutcomeCurrencies ().getRewardCurrencies (points);
	}
	 ...
	 * @return WRITEME 
	 */
	public Map <RealItem, Integer> getRewardItems (final long points) {
		return getOutcomeItems ().getRewardItems (points);
	}
	 ...
	 */
	public void setRewardByPoints (final boolean whetherToRewardByPoints) {
	 ...
	 */
	public void setRewardInventoryItemType (final RewardInventoryItemType newType) {
		rewardInventoryItemType = newType;
		changed ();
	}

	/**
	 * @param value the maximum reward scalar
	 */
	public void setRewardMax (final long value) {
		rewardMax = value;
		changed ();
	}

	/**
	 * @param value the minimum amount for the reward scalar
	 */
	public void setRewardMin (final long value) {
		rewardMin = value;
		changed ();
	}
