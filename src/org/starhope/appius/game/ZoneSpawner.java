...
public class ZoneSpawner implements AcceptsMetronomeTicks,
	...
	public static void checkZonesForSpawn () {
		...
		try {
			...
			if (Zone.lastCheckedZonesForSpawn > now
					- ZoneSpawner.getZoneSpawnSeconds () * 1000L) {
				return;
			}
			...
			AppiusClaudiusCaecus.blather ("Checking zones for spawn");
			...
			if (zones.size () < 2) {
				AppiusClaudiusCaecus
						.blather ("It is always dark in the beginning.");
			...
			int needZones = ZoneSpawner.getNumberOfZonesNeeded (zones);
			...
			if (needZones > 0) {
				...
				if (zones.size () + needZones > maxZones) {
					AppiusClaudiusCaecus
							.reportBug ("Reached maxZones of "
									+ maxZones + ", but wanted "
									+ needZones + " on top of the "
									+ zones.size () + " already up");
					needZones = Math.min (maxZones - zones.size (), 0);
				}
				if (needZones > 0) {
				...
				AppiusClaudiusCaecus
						.blather ("The number of zones is just right.");
			} else {
			...
			AppiusClaudiusCaecus.blather ("Zones good to go");
	...
	private static int getNumberOfZonesNeeded (
		...
		while (zoneIterator.hasNext ()) {
			...
			if ('$' == z.getName ().charAt (0)
				...
				AppiusClaudiusCaecus.blather ("+ZONE: " + z.getName ()
						+ " with " + numUsers
						+ " users out of a max of " + ZoneSpawner.ZONE_MAX_USERS);
				zoneCount++ ;
				totalUsers += numUsers;
		...
		final double fullNess = totalMax != 0 ? (double) totalUsers
				/ (double) totalMax : 0;
		final double lessFull = zoneCount - 1 > 0 ? fullNess
				* zoneCount / (zoneCount - 1) : Double.MAX_VALUE;

		AppiusClaudiusCaecus.blather (totalUsers
				+ " users online out of a potential maximum of "
				+ totalMax + " users; fullness="
				+ String.format ("%9.2f", Double.valueOf (fullNess)));
	...
	private static void retireUnneededZones (
		...
		AppiusClaudiusCaecus.blather ("There are about "
				+ numZonesToRemove + " too many zones.");

		// Retiring unneeded zones
		AppiusClaudiusCaecus.blather ("Retiring " + numZonesToRemove
				+ " zones");
		for (int i = 0; i < numZonesToRemove; i++ ) {
			...
			if (null != leastPopulated && zones.size () > 1) {
				AppiusClaudiusCaecus.blather ("Retiring zone "
	...
	private static void spawnNeededZones (final int needZones) {
		AppiusClaudiusCaecus.blather ("I need to spawn " + needZones
				+ " zones.");
		final Set <Thread> spawners = new HashSet <Thread> ();
		for (int i = 0; i < needZones; ++i) {
			AppiusClaudiusCaecus
					.blather ("Enqueuing spawning a zone...");
		...
		for (final Thread t : spawners) {
			try {
				...
				AppiusClaudiusCaecus
						.reportBug (
								"Caught a InterruptedException in Zone.checkZonesForSpawn ",
								e);
			}
		}
	}
	...
	public static Zone spawnZone () {
		AppiusClaudiusCaecus
				.blather ("And it came to pass in those days, that the elephants had increased greatly in numbers, and exceeded the Zones allotted to them. And so a new Zone was called for, from the rolls of Zones that might have been.");
		Quaestor.getDefault ().action (new Action ("zone.spawn.prep"));
		String zoneName = TheZones.findClaimedZoneName ();
		if ("".equals (zoneName)) {
			zoneName = TheZones.findUnclaimedZoneName ();
			if ("".equals (zoneName)) {
				Quaestor.getDefault ().action (
						new Action ("zone.overload",
								ZoneSpawner.myServer));
				AppiusClaudiusCaecus
						.blather ("No Zone names remained on the rolls for "
								+ ZoneSpawner.myServer);
				AppiusClaudiusCaecus
						.wallops (
				...
				// }
				throw AppiusClaudiusCaecus.fatalBug ("zone.overload");
			}
		}
	...
	public static Zone spawnZone (final String zoneName,
			final String image) {
		AppiusClaudiusCaecus
				.blather ("...so the Systems Programmer waved his hand across the face of the keyboard, and said: “Let there be "
						+ zoneName + "”");
		Quaestor.getDefault ().action (
				new Action ("zone.spawn", zoneName));
		...
		try {
			...
			final Room nowhere = Room
					.createPublicRoom ("nowhere", zone);
			...
			AppiusClaudiusCaecus.reportBug ("Zone spawn failed", e);
	...
	public void configUpdated () {
		...
		ZoneSpawner.USER_ROOM_MAX_USERS = AppiusConfig.getIntOrDefault (
				"org.starhope.appius.userRoom.maxUsers", 10);
		ZoneSpawner.ZONE_FULL_RATIO = Double.parseDouble (AppiusConfig
				.getConfigOrDefault (
						"org.starhope.appius.zones.fullRatio", "0.8"));
		ZoneSpawner.ZONE_LIGHT_RATIO = Double.parseDouble (AppiusConfig
				.getConfigOrDefault (
	...
	boolean freeInDB (final String zoneName) {
		...
		try {
			...
			st = con
					.prepareStatement ("UPDATE zones SET priority=1 WHERE zoneName=? AND serverName=?");
	...
	public void tick (final long currentTime, final long deltaTime)
		...
		nextCheck = currentTime
				+ AppiusConfig.getIntOrDefault (
						"org.starhope.appius.zone.spawnTime", 30000);
		return;
