...
public abstract class LaserTagGame extends GameRoom implements
	...
	public void acceptGameAction (final AbstractUser sender,
		...
		try {
			...
			LaserTagGame.log.error ("Bad gameAction from client", e);
		}
		updateScores ();
	}
	...
	protected int getLeaderBonus () {
		return AppiusConfig
				.getIntOrDefault (
						"org.starhope.appius.game.LaserTagGame.leaderBonus",
						0);
	}
	 ...
	 * @return the number of peanuts earned for hitting the given
	 *         target
	 */
	protected abstract long getScoreForObject (String type, int subtype);
	...
	private void shotObject (final AbstractUser sender,
		...
		final String [] typeParts = action.getString ("type").split (
				"~");
		if (typeParts.length != 2) {
			LaserTagGame.log
					.error ("User shot object: can't interpret type: "
			...
			final int subType = Integer.parseInt (typeParts [1]);
	 ...
	 * @throws JSONException WRITEME
	 */
	private void shotPVP (final JSONObject action)
			throws JSONException {
		final AbstractUser shooter = Nomenclator
				.getUserByLogin (action.getString ("shooter"));
		final int shooterID = shooter.getUserID ();
		if (players.contains (Integer.valueOf (shooterID))) {
			incrementScore (shooter,
					AppiusConfig.getIntOrDefault (getClass ()
							.getCanonicalName () + ".hitPlayer",
							10));
		}
		final AbstractUser victim = Nomenclator
				.getUserByLogin (action.getString ("victim"));
		final int victimID = victim.getUserID ();
		if (players.contains (Integer.valueOf (victimID))) {
			decrementScore (victim,
					AppiusConfig.getIntOrDefault (getClass ()
							.getCanonicalName () + ".hitDeduct",
							3));
		}
	}
