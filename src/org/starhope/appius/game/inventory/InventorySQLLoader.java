...
public class InventorySQLLoader implements
	 ...
	 * </p>
	 * <p>
	 * Note that inventories are auto-vivified if not found. This loader
	 * will <em>not</em> return a {@link NotFoundException}
	 * </p>
	 * 
	...
	public Inventory loadRecord (final int userID) {
		...
		try {
			...
			st = con
					.prepareStatement ("SELECT slot FROM inventory WHERE userID=?");
			Inventory inventory;
			st.setInt (1, userID);
			rs = st.executeQuery ();
			...
			for (Integer slot : slots) {
				try {
					inventory.add (Nomenclator.getDataRecord (
							InventoryItem.class, slot.intValue ()));
			...
			AppiusClaudiusCaecus.reportBug (
					"Caught a SQLException in Inventory.fetch ", e);
		} finally {
	...
	public void saveRecord (
		...
		Iterator <InventoryItem> i = record.iterator ();
		while (i.hasNext ()) {
