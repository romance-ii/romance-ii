...
SimpleDataRecord <InventoryItemType> {
	...
	public static InventoryItemType get (final String string) {
		try {
			return Nomenclator.getDataRecord (InventoryItemType.class,
					string);
	...
	public InventoryItemType () {
		...
		valences.add (new Pair<Integer,Integer>(Integer.valueOf (0), Integer.valueOf(1)));
	}
	...
	/**
	 * Determine whether this slot permits a certain number of items to be mounted in non-conflicting valence levels, indicated by the Z depth of the items. An item type that only allows one instance per slot will have a single valence level (e.g. perhaps “hair”), while an item type that might have a couple of distinct levels which could conflict only within a valence level (e.g. under-shirt, shirt, vest, jacket, coat) would have multiples.
	 * @return true, if this item type respects valence levels (limits the number of items mounted to it). False indicates that any number of items can be equipped in this slot, without limits.
	 */
	 ...
	 * @return whether having an item of this type equipped should prevent equipping another item of this type
	 */
	...
	/**
	 * Remove valence effects from this type; set it to have a single valence level (at zero) permitting
	 * {@link Integer#MAX_VALUE} items
	 */
	public void removeValences () {
		hasValences=false;
		valences.clear ();
		valences.add (new Pair<Integer,Integer>(Integer.valueOf (0), Integer.valueOf(Integer.MAX_VALUE)));
	}
	...
	/**
	 * Set this type to a singleton valence: one valence level (at zero) permitting only a single item.
	 */
