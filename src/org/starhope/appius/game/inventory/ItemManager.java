...
public class ItemManager {
	...
	public static ItemManager get (final AbstractUser u)
		...
		return ItemManager.get (u, u.getAvatarClass (), u
				.getInventory ());
	}
	...
	private void addEffects (final InventoryItem item) {
		if (EquipType.SILENT == item.getGenericItem ().getEquipType ()) {
			return;
		}
		try {
	 ...
	 * @param item WRITEME
	 * @return WRITEME
	 * @throws AlreadyUsedException if the item can't be equipped due to
	 *             lack of a slot on the avatar for items of this type
	 */
	private Collection <InventoryItem> getItemConflicts (
		...
		for (final InventoryItem equipped : getInventory().getEquippedItems (true)) {
			try {
				if (equipped.getItemEffects ().conflicts (itemClass,
						item)) {
	...
	private Collection <InventoryItem> getSlotConflicts (
		...
		if (itemType.hasValences ()) {
			...
			conflicts.addAll (getInventory().getItemsByType (itemType));
	 ...
	 * effects should be removed, and also remove any items that require
	 * this one to be equipped. If this routine returns “false,” then it
	 * is impossible to resolve conflicts, and the item must not be
	 * de-equipped.
	 *
	 ...
	 * effects should be applied, and conflicting items removed. If this
	 * routine returns “false,” then it is impossible to resolve
	 ...
	 */
	public void useEquipment (final char slot, final String targetName,
			final Coord3D targetCoords) {
		Geometry.updateUserPositioning (owner);
		for (InventoryItem item : inv.getEquippedItems (false)) {
