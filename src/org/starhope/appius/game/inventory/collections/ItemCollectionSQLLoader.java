...
public class ItemCollectionSQLLoader implements
	...
	@Override
	public void refresh (final ItemCollection record) {
	...
	public void removeRecord (final ItemCollection record) {
		...
		BugReporter
				.getReporter ("srv")
				.reportBug (
						"unimplemented RecordLoader<ItemCollection>::removeRecord (brpocock@star-hope.org, Jul 22, 2010)");
	...
	public ItemCollection set (final PreparedStatement selectHeader,
		...
		try {
			...
			r.setOnBreakEvent (Nomenclator.getDataRecord (
					EventType.class, header.getInt ("onBreak")));
			r.setOnCompleteEvent (Nomenclator.getDataRecord (
					EventType.class, header.getInt ("onComplete")));
			items = selectItems.executeQuery ();
			while (items.next ()) {
				try {
					r.add (Nomenclator.getDataRecord (
