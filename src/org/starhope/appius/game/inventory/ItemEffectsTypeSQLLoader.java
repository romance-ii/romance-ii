...
public class ItemEffectsTypeSQLLoader implements
	...
	public ItemEffectsType loadRecord (final int id)
		...
		try {
			...
			st = con
					.prepareStatement ("SELECT * FROM itemEffectsClasses WHERE id=?");
			st.setInt (1, id);
			rs = st.executeQuery ();
			rs.next ();
			ItemEffectsType body = new ItemEffectsType (this);
			return load (body, rs);
	...
	public ItemEffectsType loadRecord (final String identifier)
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("SELECT * FROM itemEffectsClasses WHERE klass=?");
	...
	public void removeRecord (final ItemEffectsType record) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("DELETE FROM itemEffects WHERE id=?");
			st.setInt (1, record.getID ());
			rs = st.executeQuery ();
		} catch (SQLException e) {
			AppiusClaudiusCaecus
					.reportBug (
							"Caught a SQLException in ItemEffectsTypeSQLLoader.removeRecord ",
							e);
		} finally {
			LibMisc.closeAll (st, rs, con);
		}
	...
	public void saveRecord (final ItemEffectsType record) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("INSERT INTO itemEffects (id, klass) VALUES (?,?) ON DUPLICATE UPDATE SET klass=Values(klass) WHERE id=Values(id)");
			st.setInt (1, record.getID ());
