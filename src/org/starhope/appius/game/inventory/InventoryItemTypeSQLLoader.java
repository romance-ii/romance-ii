...
public class InventoryItemTypeSQLLoader implements
	...
	public InventoryItemType loadRecord (final int id)
		...
		try {
			...
			st = con
					.prepareStatement ("SELECT id, name FROM itemTypes WHERE id=?");
			st.setInt (1, id);
			rs = st.executeQuery ();
			return get (rs);
	...
	public InventoryItemType loadRecord (final String identifier)
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("SELECT id, name FROM itemTypes WHERE name=?");
	...
	public void removeRecord (final InventoryItemType record) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("DELETE FROM itemTypes WHERE id=?");
			st.setInt (1, record.getID ());
			st.executeUpdate ();
		} catch (SQLException e) {
	...
	public void saveRecord (final InventoryItemType record) {
		...
		System.err
				.println ("Not saving InventoryItemType record change");
	}
