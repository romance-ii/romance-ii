...
public class GenericItemReference extends
		SimpleDataRecord <GenericItemReference> implements AbstractItem {
	 ...
	 * @return WRITEME
	 * @deprecated user {@link Nomenclator#getDataRecord(Class, int)}
	 */
	@Deprecated
	public static GenericItemReference getByID (final int itemID) {
		try {
	...
	public static GenericItemReference getRandomItem (
			final RarityRating minRarity, final RarityRating maxRarity) {
		...
		do {
			try {
				/*
				 * try random numbers for a long time. if that fails,
				 * fall back on iterating through the set of all items.
				 */
				...
				if (item.getRarity ().compareTo (minRarity) < 0
						|| item.getRarity ().compareTo (maxRarity) > 0) {
					item = null;
				}
			} catch (final NotFoundException e) {
			...
			if (termination > maxID) {
				throw AppiusClaudiusCaecus
						.fatalBug ("There are no items in range ("
								+ minRarity.getValue () + ".."
								+ maxRarity.getValue () + ")");
			}
		} while (null == item);
	...
	/**
	 * The ID of the {@link ItemEffectsType} to determine the {@link ItemEffects} for this item.
	 */
	 ...
	 *
	 * @see org.starhope.appius.game.inventory.AbstractItem#getCollections()
	 */
	@Override
	public Collection <ItemCollection> getCollections () {
		final Collection <ItemCollection> all = new HashSet <ItemCollection> ();
		Connection con = null;
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("SELECT collectionID FROM itemCollectionItems WHERE itemID=?");
			st.setInt (1, itemID);
			rs = st.executeQuery ();
			while (rs.next ()) {
				final int collectionID = rs.getInt (1);
				try {
					all.add (Nomenclator.getDataRecord (
							ItemCollection.class, collectionID));
				} catch (final NotFoundException e) {
					AppiusClaudiusCaecus
							.reportBug (
									"item #"
											+ itemID
											+ " is a member of non-existing collection "
											+ collectionID, e);
				}
			}
		} catch (final SQLException e) {
	...
	/**
	 * @return the ID of the {@link ItemEffectsType} to determine the {@link ItemEffects} for this item.
	 */
	...
	/**
	 * set the {@link ItemEffects} class type ID pointer (via {@link ItemEffectsType} table indirection) for this item type. Note that if the item is currently set to return a {@link #getEquipType()} of {@link EquipType} {@link EquipType#SILENT}, setting a non-null handler will force it to {@link EquipType#PASSIVE}. Likewise, setting this to a -1 (null) handler ID will revert to {@link EquipType#SILENT}.
	 * @param newEffectsType the ID of the new {@link ItemEffectsType} to be applied
	 ...
	 * inventory slots (see {@link InventoryItem#getSlotNumber()}, which
	 ...
	 * ({@link InventoryItemType}), which are used to define slots and
	 ...
	 * health indicator.
	 *
	 * @param newHealthType The measurement type to be used to interpret
	 ...
	 */
	public void setItemType (final InventoryItemType inventoryItemType) {
	 ...
	 * @param int1 WRITEME
	 */
	public void setPrice (final Currency units, final int int1) {
		setPrice (units, BigDecimal.valueOf (int1));
	}
