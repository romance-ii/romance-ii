...
public class GenericItemReferenceSQLLoader implements
	...
	private static final Logger log = LoggerFactory
			.getLogger (GenericItemReferenceSQLLoader.class);
	...
	public GenericItemReference loadRecord (final int id)
		...
		try {
			...
			st = con.prepareStatement ("SELECT * FROM items LEFT JOIN itemEffects ON items.ID=itemEffects.itemID WHERE ID=?");
			st.setInt (1, id);
			rs = st.executeQuery ();
			if (rs.next ()) {
				return set (rs);
	...
	public GenericItemReference loadRecord (final String identifier)
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("SELECT * FROM items LEFT JOIN itemEffects ON items.ID=itemEffects.itemID WHERE name=?");
	...
	private GenericItemReference reload (final ResultSet rs,
		...
		synchronized (rec) {
			...
			try {
				rec.setInventoryItemType (Nomenclator
			...
			rec.setPrice (Currency.getPeanuts (),
					rs.getInt ("value"));
			
			String equipSlotString = rs.getString ("equipSlot");
			if (equipSlotString.length () < 1) {
				equipSlotString = " ";
			}
			char equipSlot = equipSlotString.charAt (0);
			if ( (equipSlot < ' ')
					|| ( (equipSlot >= 0x7f) && (equipSlot <= 0xa0))) {
				equipSlot = ' ';
			}
			...
			final char healthType = healthTypeStr.charAt (0);
			switch (healthType) {
			case 'C':
			...
			/*
			 * And now, check for a trailer record (for effects).
			 * Since it's created on a LEFT JOIN, this can be a null
			 * column.
			 */
	...
	public void removeRecord (final GenericItemReference record) {
		...
		BugReporter
				.getReporter ("srv")
				.reportBug (
						"unimplemented RecordLoader<GenericItemReference>::removeRecord (brpocock@star-hope.org, Jul 22, 2010)");
	...
	public void saveRecord (final GenericItemReference record) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("INSERT INTO items (itemTypeID, name, description, itemRarity, CanTrade, value, ID) VALUES (?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE ID=Values(ID), itemTypeID=Values(itemTypeID), name=Values(name), description=Values(description), itemRarity=Values(itemRarity), CanTrade=Values(CanTrade), value=Values(value)");
			st.setInt (1, record.getInventoryItemType ().getID ());
