...
public class InventoryItem extends SimpleDataRecord <InventoryItem>
	 ...
	 * @param idByWhichToGet the database ID of an inventory item of any
	 *            kind
	...
	private void checkEquipChange () {
		...
		if (null == item) {
			AppiusClaudiusCaecus
					.reportBug ("Item ID doesn't resolve to a GenericItemReference?"
							+ itemID);
			return;
		}
	...
	public InventoryItem copyProtoype (final InventoryItem prototype) {
		...
		colours.clear ();
		for (Colour colour : prototype.colours) {
			colours.add (colour);
		}
	...
	private void reportBug (final String bug, final Throwable e) {
		if (null == e) {
			AppiusClaudiusCaecus.reportBug (bug
					+ " (InventoryItem slot#" + slotNumber + ")");
		} else {
			AppiusClaudiusCaecus.reportBug (bug
					+ " (InventoryItem slot#" + slotNumber + ")", e);
		}
	}
	...
	public void setActive (final boolean beActive) {
		...
		try {
			Quaestor.getDefault ()
					.action (
							new Action (getOnlineOwner (),
									(active ? "don" : "doff"), String
											.valueOf (itemID), health));
		} catch (final NotFoundException e) {
			// no op
		}
		checkEquipChange ();
	...
	public void setOwnerID (final int newOwnerID) {
		...
		changed ();
		// Don't Nomenclator.getUserByID (ownerID1).getInventory ().add
		// (this);
	}
	...
	public void setRoom (final Room room) {
		...
		if ( !room.getOwner ().equals (getOwner ())) {
			reportBug (
					"Attempting to place item into another person's room : item "
							+ getSlotNumber () + " (owner "
							+ getOwner ().getDebugName () + ")  in "
							+ room.getDebugName () + " (owner "
		...
		try {
			final InventoryItemType wallpaperItemType = Nomenclator
					.getDataRecord (InventoryItemType.class,
							"Wallpaper");
			if (type.equals (wallpaperItemType)) {
				final HashSet <InventoryItemType> set = new HashSet <InventoryItemType> ();
				set.add (wallpaperItemType);
		...
		try {
			final InventoryItemType flooringItemType = Nomenclator
					.getDataRecord (InventoryItemType.class, "Flooring");
			if (type.equals (flooringItemType)) {
				final HashSet <InventoryItemType> set = new HashSet <InventoryItemType> ();
				set.add (flooringItemType);
				for (final InventoryItem item : getOwner ()
						.getInventory ().getItemsByType (set)) {
					if ( !equals (item)
							&& item.getRoomIndex () == getRoomIndex ()) {
		...
		try {
			final InventoryItemType ceilingItemType = Nomenclator
					.getDataRecord (InventoryItemType.class, "Ceiling");
			if (type.equals (ceilingItemType)) {
				final HashSet <InventoryItemType> set = new HashSet <InventoryItemType> ();
				set.add (ceilingItemType);
				for (final InventoryItem item : getOwner ()
						.getInventory ().getItemsByType (set)) {
					if ( !equals (item)
							&& item.getRoomIndex () == getRoomIndex ()) {
	 ...
	 * flushing the item.
	 *
	 * @param slotNumber_force The retrieved slot number from an insert.
	 */
	...
	public void setTypeID (final int newTypeID)
		...
		try {
			type = Nomenclator.getDataRecord (InventoryItemType.class,
					newTypeID);
		} catch (final NotFoundException e) {
			AppiusClaudiusCaecus
					.reportBug (
	...
	public JSONObject toJSON () {
		...
		try {
			...
			self.put ("ownerID", null == owner ? -1 : owner
					.getUserID ());
