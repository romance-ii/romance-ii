...
public class BugReporter {
	...
	public static BugReporter getReporter (final String purpose) {
		...
		try {
			...
			try {
				...
				System.err.flush ();
				return null; // unreachable, but makes compiler
				// happy.
	...
	static String getStackTrace (final Throwable throwable,
		...
		for (final StackTraceElement element : throwable
			...
			if (80 < ++watchdog) {
				stackTrace.append (prefix);
				stackTrace.append ("... and many more (limiter invoked)");
	...
	private BugReporter (final String newPurpose)
		...
		purpose = newPurpose;
		final BugReporter old = BugReporter.reporters.put (purpose,
				this);
		if (null != old) {
		...
		setMailFrom (AppiusConfig.getConfigOrDefault (confPrefix
				+ "mailFrom", AppiusConfig.getMailSender ()
				.toString ()));
		setSubjectTagline (AppiusConfig.getConfigOrDefault (
				confPrefix + "subject", "Appius Claudius Caecus"));
		setIncludeStackTrace (AppiusConfig
				.getConfigBoolOrTrue (confPrefix
						+ "includeStackTrace"));
	...
	public void bugDuplex (final String subject, final String message) {
		...
		// Version version = xxx;
		// TracClientFactory.createClient(abstractWebLocation,
		// version)
	 ...
	 * 
	 * @param aString WRITEME  brpocock 
	 * @return WRITEME  brpocock 
	 */
	public Error fatalBug (final String aString) {
		throw fatalBug (new Exception (aString));
	}
	...
	public void reportBug (final String reason, final Thread thread) {
		...
		bugReport.append ("/* Bug report auto-generated by software.\n/= Reason: ");
	...
	public void reportBug (final String reason,
		...
		while (null != cause) {
			...
			bugReport.append (prefix);
			bugReport.append (" Exception was caused by another exception");
			bugReport.append (prefix);
			...
			bugReport.append (BugReporter.getStackTrace (cause,
					prefix + "# "));
		...
		bugReport.append ("\n/= Timestamp Now: "
				+ "\n/= Human Timestamp: ");
		bugReport.append (AppiusClaudiusCaecus.isoDate
				.format (Calendar.getInstance ().getTime ()));
		bugReport.append ("\n/> End automated bug report");
		AppiusClaudiusCaecus.bugDuplex (reason + " — "
				+ subjectTagline, bugReport.toString ());
	...
	private void sendBugReport (final String subject,
		...
		if ( (null == mailFrom) || (null == mailTo)) {
			System.err.println (" [×] Can't send mail: sender or recipient is null.");
			return;
		}
		
		final String messagePlusTrailer = message
		...
		try {
			...
			msg.setHeader ("X-Generator",
					BugReporter.class.getCanonicalName () + "/"
			...
			report.append ("Unable to send the following message because of an exception in sending the eMail.\n\nMessage:\n\nSubject: ");
	...
	public void setIncludeStackTrace (
			final boolean nowIncludeStackTrace) {
	...
	private boolean tooManyBugsMailed () {
		...
		for (final Long when : AppiusClaudiusCaecus.bugsMailed) {
