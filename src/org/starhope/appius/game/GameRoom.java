 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
 */
public abstract class GameRoom extends GameBase implements RoomListener {
	...
	public static void propagateGameStateChange () {
		try {
			while (true) {
				...
				GameRoom.log.debug ("game state pump firing game "
						+ ev.toString () + " to "
						+ state.toString ());
	...
	public void acceptCommand (final GeneralUser user,
		...
		if ("#resetplayers".equals (command [0])) {
			resetPlayers ();
			user.acceptMessage ("resetPlayers "
					+ getGameShortName (), "DM",
					"resetPlayers: OK");
			return;
		}
		...
		if ("#scores".equals (command [0])) {
			...
			msg.append (toString ());
			msg.append (")\n");
			for (final Entry <AbstractUser, Long> score : scores
					.entrySet ()) {
		...
		if ("#purge".equals (command [0])) {
			...
			user.acceptMessage ("Purge", "DM", "Purged "
					+ purgedNull + " nulls and " + purgedOffline
		...
		if (newListener instanceof AbstractUser) {
			...
			if (0 != Integer.parseInt (playerTag)) {
				...
				if ( !scores.containsKey (newUser)) {
					scores.put (newUser, 0L);
				}
				if ( (players.size () > 1)
						&& (GameStateFlag.GAME_SOLO == gameState)) {
					// start game play when 2º player has just
					// entered
	...
	public void destroySelf () {
		// log.debug ("Destroying myself! " + getName
		// () + "@" + getZone ().getName ());
	...
	/**
	 * Time (in ms) for the game play period timer. Can be overridden
	 * by derived classes.
	 * 
	 ...
	 * @param score2 increment amount, can be negative
	 */
	public void incrementScore (final AbstractUser who,
		...
		long score = scores.containsKey (who) ? scores.get (who)
				.longValue () : 0L;
		score = Math.max (0L, score + score2);
		scores.put (who, new Long (score));
		updateRoomVars ();
	...
	/**
	 * Clear all players; clear all scores; put all users into player
	 * or spectator queues
	 */
	protected void resetPlayers () {
		...
		for (final Room room : rooms) {
			for (final AbstractUser user : room.getAllUsers ()) {
				...
				if ( (null != isPlayer)
						&& (Integer.parseInt (isPlayer) > 0)) {
					final Integer userID = Integer.valueOf (user
							.getUserID ());
					players.add (userID);
					scores.put (user, Long.valueOf (0));
				}
			}
		}
		updateScores ();
	...
	protected void sendEndEvents (final String gameMoniker) {
		...
		for (final int i : players) {
			...
			if (scores.containsKey (userByID)) {
				final int thisScore = scores.get (userByID)
						.intValue ();
		...
		for (final int id : players) {
			...
			if (null != user) {
				...
				if (playerEvents.containsKey (Integer
						.valueOf (userID))) {
					/*
					 * Check all parameters, very paranoid
					 */
					final Integer eventID = playerEvents
					...
					if ( (null != eventID)
						 ...
						 */
						final ConcurrentHashMap <AbstractUser, Long> scoresCopy = new ConcurrentHashMap <AbstractUser, Long> ();
						scoresCopy.putAll (scores);
						final LinkedHashMap <AbstractUser, Long> sortedScores = LibMisc
								.sortHashMapByValues (scoresCopy);
						
						EventRecord event;
						try {
							event = Nomenclator.getDataRecord (
									EventRecord.class,
									eventID.intValue ());
						} catch (final NotFoundException e) {
							GameRoom.log
									.error ("Caught a NotFoundException in GameEvent.sendEndEvents Quaestor.getEventByID",
											e);
						}
					}
				} else {
	...
	private void sendScoreUpdate (final AbstractUser player,
			final long score) {
		int place = 1;
		for (final long i : scores.values ()) {
			if (i > score) {
				++place;
			}
		}
		...
		try {
			msg.put ("score", score);
			msg.put ("place", place);
			player.acceptSuccessReply ("scoreUpdate", msg,
					player.getRoom ());
		} catch (final JSONException e) {
			GameRoom.log.error ("Exception", e);
		}
	...
	protected void sendStartEvents (final String gameMoniker) {
		for (final int id : players) {
			...
			if (null != user) {
				...
				// Quaestor.startEvent_JSON (user, gameMoniker);
				// user.acceptSuccessReply ("startEvent", result,
				// user.getRoom ());
				//
				// playerEvents.put (Integer.valueOf (user.getUserID
				// ()),
				// Integer.valueOf (result.getInt ("eventID")));
				// } catch (final JSONException e) {
	...
	protected void updateRoomVars () {
		...
		case GAME_RUNNING:
			String leaderName = "";
			long leaderScore = 0L;
			for (final Entry <AbstractUser, Long> who : scores
					.entrySet ()) {
				final long playerScore = who.getValue ()
						.longValue ();
		...
		if ( (null != who) && (who instanceof GeneralUser)) {
			...
			if (null != playerThread) {
				...
				if (scores.containsKey (who)) {
					final long score = scores.get (who);
					sendScoreUpdate (who, score);
				}
			}
		}
