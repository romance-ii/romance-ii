...
public abstract class MultiPlayerMiniGame extends GameRoom {
	...
	/**
	 * the lobby room is instanced off for individual games, but used
	 * to negotiate game joins.
	 */
	protected final Room lobbyRoom;
	
	/**
	 * the class of game events for instance rooms to invoke. Must have
	 * a constructor taking “this” and the room as parameters (in that
	 * order). Needs to be strongly typed to take the implementing
	 * subclass…
	 */
	 ...
	 * @param roomEvent minigame handler class —
	 *             {@link #roomEventClass}
	 * @throws NotFoundException if the given room doesn't exist
	 */
	protected MultiPlayerMiniGame (final Zone z, final char c,
			final String moniker,
			final Class <? extends GameRoom> roomEvent)
				...
				try {
					final GameRoom newGame = roomEventClass
							.getConstructor (this.getClass (),
									Room.class).newInstance (
									this, channel);
					newGame.acceptObjectJoinChannel (channel,
							newListener);
