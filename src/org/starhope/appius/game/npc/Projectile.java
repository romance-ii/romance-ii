...
public class Projectile extends Ejecta {
	...
	private static final class ProjectileWatcher implements
		...
		public ProjectileWatcher (final Projectile projectile) {
			...
			polygon = myProjectile.getAvatarClass ()
					.getCollisionBounds ().scale (
							myProjectile.getSizeScalar ());
		}
		...
		public String getName () {
			return "ProjectileWatcher/" + myProjectile.getDebugName ();
		}
		...
		public void tick (final long currentTime, final long deltaTime)
			...
			for (final AbstractUser candidate : myProjectile.getRoom ()
					.getAllUsers ()) {
				...
				final PolygonPrimitive <?> candidatePoly = candidate
						...
						.translate (
								candidate.getLocation ().toCoord2D ());
	...
	public Projectile (final AvatarClass avatar,
			final AbstractUser shooter, final AvatarClass hitParticle,
			final AvatarClass missParticle, final DamageTypeRanks damage) {
	 ...
	 */
	@Override
	public void fire (Room room, Coord3D origin, long birth,
			Coord3D target, double travelRate) {
	...
	public void miss () {
		...
		if (null != myMissParticleAvatar) {
			new Particle (myMissParticleAvatar)
					.fire (getRoom (), getLocation (),
							System.currentTimeMillis (),
							getLocation (), 0,
							(long) (1000 / getTravelRate ()));
