 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
	...
	public Queue <Runnable> accept (final Action a) {
		if (myTest.matches (a, myUser)) {
			return myOutcome.getSteps ();
		}
		return null;
	}
	...
	public ActionHandlerInterface handler (
		...
		Room room;
		try {
			room = null == locative ? null : scriptRunner
					.getPuppet ().getZone ()
					.getRoomByName (locative);
		...
		final AbstractUser subject = null == nominative ? null
				: Nomenclator.getUserByLogin (nominative);
		final String accusative = myTest.getAccusative ();
		final AbstractUser object = null == accusative ? null
				: Nomenclator.getUserByLogin (accusative);
		return new ActionHandler (room, subject, myTest.getVerb (),
				object, myTest.getDative (),
				new HackSetToActionRunner (scriptRunner));
	}
