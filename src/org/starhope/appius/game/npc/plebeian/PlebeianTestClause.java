...
public class PlebeianTestClause {
	 ...
	 * @return WRITEME
	 */
	public PlebeianTestClause addLocativeProposition (
			final String loc) {
		return new PlebeianTestClause (verb, nominative, accusative,
				dative, loc);
	}
	 ...
	 * 
	 * @param v verb
	 * @return test clause based upon this one with the verb proposition
	 *         added
	 */
	...
	public boolean matches (final Action a, final AbstractUser whoAmI) {
		if (null != verb && !a.getVerb ().equals (verb)) return false;
		...
		if ("Myself".equals (nominative) && whoAmI.equals (subject)) {
			...
			if (null != nominative
					&& null != subject
					&& !AbstractNonPlayerCharacter.getNameStripped (
							subject.getAvatarLabel ()).toLowerCase (
									Locale.ENGLISH).equals (
											nominative.toLowerCase (Locale.ENGLISH))) return false;
		...
		if ("Myself".equals (accusative) && whoAmI.equals (object)) {
			...
			if (null != accusative
					&& null != object
					&& !AbstractNonPlayerCharacter.getNameStripped (
							object.getAvatarLabel ()).toLowerCase (
									Locale.ENGLISH).equals (
											accusative.toLowerCase (Locale.ENGLISH))) return false;
		final Room where = a.getWhere ();
		if ("Here".equals (locative)
				&& whoAmI.getRoom ().equals (where)) {
			// OK so far…
		} else
			if (null != locative && null != where
					&& !where.getName ().equals (locative)) return false;
		final String indirectObject = a.getIndirectObject ();
		if (null != dative && null != indirectObject
				&& !indirectObject.equals (dative)) return false;
		return true;
	}
	...
	public String toString () {
		...
		+ accusative + " dat." + dative + " loc." + locative
		+ ")";
	}
