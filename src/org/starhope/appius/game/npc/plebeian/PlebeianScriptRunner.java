...
Serializable {
	...
	private final class GoToNamedTargetRunner implements Runnable {
		...
		public String toString () {
			...
			+ " to region " + destination + " via " + moveVerb;
		}
	}
	...
	private final class GoToRoomRunner implements Runnable {
		 ...
		 * WRITEME: Document this constructor brpocock@star-hope.org
		 *
		 * @param goalRoomMoniker the moniker of the room to which to go
		 */
	...
	private final class GrantToActorRunner implements Runnable {
		 ...
		 */
		GrantToActorRunner (final int itemID, final PlebeianActor actor) {
			item = itemID;
			rôle = actor;
		}
	...
	final class HackPropsPrompt implements Runnable {
		...
		public void run () {
			...
			// }));
			//
			// shopper.acceptSuccessReply ("prompt", prompt, getPuppet
			// ()
			// .getRoom ());
			...
			shopper.acceptSuccessReply ("overlayWindow", propsOverlay,
					getPuppet ().getRoom ());
		}
	}
	...
	final class HackPropsShootThem implements Runnable {
		...
		public void run () {
			final AbstractUser shopper = getActorByRole (PlebeianActor.Subject);
			if (getPuppet ().equals (shopper)) return;
			try {
				for (final InventoryItem i : getPuppet ().getInventory ()
						.getItemsByType (
	...
	private final class PaySomeoneRunner implements Runnable {
		...
		@Override
		public void run () {
			getActorByRole (rôle).getWallet ().add (currency, amount);
		}
	...
	public final class Random_Runner implements Runnable {
		 ...
		 * WRITEME: Document this constructor brpocock@star-hope.org
		 *
		 * @param options a set of scripts, from which one outcome will
		...
		public void run () {
			choices.get (
					AppiusConfig.getRandomInt (0, choices.size () - 1))
					.nowRun (getPuppet ());
		}
	 ...
	 * @author brpocock@star-hope.org
	 */
	private final class SimultaneousExecutionRunner implements Runnable {
		/**
		 * WRITEME: Document this brpocock@star-hope.org
		 */
	...
	PlebeianScriptConditional {
		 ...
		 */
		TestActorEquippedItem (final PlebeianActor actor, final int item) {
			rôle = actor;
			itemID = item;
		}
		...
		public boolean evaluate () {
			return getActorByRole (rôle).getInventory ().hasEquipped (
					itemID);
		}
	...
	/**
	 * Test whether the current Action's indirect object string contains
	 * a certain (case-insensitive) substring.
	 * 
	 * @author brpocock@star-hope.org
	 */
	...
	PlebeianScriptConditional {
		...
		public boolean evaluate () {
			...
			if (null == peekAction) {
				toDo.clear ();
				AppiusClaudiusCaecus
				.reportBug ("Got a null action on the stack: clearing script TO-DOs");
				return false;
			}
			final String indirectObject = peekAction.getIndirectObject ();
			if (null == indirectObject) return false;
			return indirectObject.toLowerCase (
					Locale.ENGLISH).contains (keyword);
		}
	...
	PlebeianScriptConditional {
		...
		/**
		 * Perform two sub-tests using a short-circuited Boolean logical
		 * OR operation, returning true if either the first or second
		 * return true (but not evaluating the second if the first
		 * succeeds)
		 * 
		 * @param head a test
		 * @param tail another test
	...
	/**
	 * whitespace and whitespace-equivalent punctuation characters to be
	 * ignored in tokenizing
	 */
	...
	private PlebeianScriptConditional addCondition (
		...
		if (null == conj || null == prior) return latter;
		if (null == latter) return prior;
		switch (conj) {
			case and:
	...
	/**
	 * Clear all scripted content, usually in preparation for a new script to be loaded
	 */
	...
	public void compileScript (final String scriptTextString) {
		final StringTokenizer tokens = new StringTokenizer (scriptTextString, " \t\n", false);
		while (tokens.hasMoreTokens ()) {
			try {
				final String token = tokens.nextToken ();
				parseDeclarationCommand (token, tokens);
			} catch (final GameLogicException e) {
				AppiusClaudiusCaecus.reportBug ("Script error in "
						+ toString (), e);
			}
		}
	}
	...
	synchronized public void dispatch (final Action a) {
		if (getPuppet ().equals (a.getSubject ())) return;
		...
		final Queue <PlebeianExpression> always = expressions.get ("");
		if (null != always) {
	...
	protected AbstractUser getActorByRole (final PlebeianActor rôle) {
		...
		switch (rôle) {
			case Myself:
				return getPuppet ();
			case Object:
				return null == peekAction ? null : peekAction.getObject ();
			case Subject:
				return null == peekAction ? null : peekAction.getSubject ();
		}
		throw AppiusClaudiusCaecus.fatalBug ("Actor type enum failure");
	}

	/**
	 * WRITEME: Document this method brpocock@star-hope.org
	 *
	 * @return time at which the last scripted action should have / will
	 *         complete
	 */
	 ...
	 * 
	 * @param login the NPC's user name (also used to find their script)
	 * @throws GameLogicException usually on a syntax error or something
	 */
	...
	protected void nextToDoItemSoon () {
		final long now = System.currentTimeMillis ();
		if (lastScriptTime > now) return;
		lastScriptTime = AppiusClaudiusCaecus.getKalendor ().schedule (
				now + 60, new RunNextStep (this));
	}
	...
	private PlebeianScriptConditional parseConditional (
		...
		while (true) {
			...
			if ("contain".equals (token) || "contains".equals (token)) {
			...
			if ("has".equals (token)) {
				token = tokens.nextToken ();
				final PlebeianActor rôle = PlebeianActor.valueOf (stack
						.remove ());
				if ("item".equals (token)) {
					...
					try {
						final Currency cu = Nomenclator.getDataRecord (
								Currency.class, tokens.nextToken ());
						final BigDecimal qty = new BigDecimal (tokens
								.nextToken ());
						prior = addCondition (
								prior,
								conj,
								new TestActorHasCurrency (rôle, cu, qty));
					} catch (final NotFoundException e) {
						throw new GameLogicException (
								"currency not recognized", this, e);
					}

				} else throw new GameLogicException ("has what?", this,
						token);
				continue;
				...
				final PlebeianActor rôle = PlebeianActor.valueOf (stack
						.remove ());
				if ("wearing".equals (token) || "using".equals (token)) {
					final int itemID = Integer.parseInt (tokens
							.nextToken ());
					prior = addCondition (prior, conj,
							new TestActorEquippedItem (rôle, itemID));
				}
				continue;
			}
			if ("then".equals (token)) return prior;
			if ("and".equals (token)) {
	...
	private void parseDeclarationCommand (final String token,
		...
		try {
			...
			if ("Include".equals (token)) {
				readScriptFile (new File ("/etc/appius/npc/include/"
						+ tokens.nextToken ()));
				return;
			}
		...
		while (true) {
			...
			if ("Go".equals (verb)) {
				...
				if ( ! (dest.charAt (0) >= 'a'
					&& dest.charAt (0) <= 'z'
						|| dest.charAt (0) >= 'A'
							&& dest.charAt (0) <= 'Z' || dest.charAt (0) == '$')) {
					final Coord3D destCoord = Coord3D.fromString (dest
							+ "," + next + "," + tokens.nextToken ());
					next = tokens.nextToken ();
					if ("via".equals (next)) {
				...
				final int itemID = Integer.parseInt (tokens
						.nextToken ());
				if ( !"to".equals (tokens.nextToken ())) throw new GameLogicException ("Give item to whom?",
						this, steps);
				final String target = tokens.nextToken ();
				step = new GrantToActorRunner (itemID, PlebeianActor
						.valueOf (target));
				next = tokens.nextToken ();
				...
				final BigDecimal amount = new BigDecimal (tokens
						.nextToken ());
				next = tokens.nextToken ();
				...
				final int waitTime = (int) (Double.parseDouble (tokens
						.nextToken ()) * 1000);
				step = new Runnable () {
				...
				if ("else".equals (next) || "otherwise".equals (next)) {
				...
				next = tokens.nextToken ();
					...
					"Unexpected verb in script section: " + verb,
					this, steps);
			...
			if ("while".equals (next) || "and".equals (next)
					|| "meanwhile".equals (next) || "&".equals (next)
					|| ";".equals (next)) {
				prepend.add (step);
				continue;
			}
			if ("then".equals (next) || "next".equals (next)
					|| "afterwards".equals (next) || ".".equals (next)
			...
			throw new GameLogicException (
					"Unexpected conjunction in script section: " + next,
					this, steps);
		}
	}
	...
	throws GameLogicException {
		...
		while (true) {
			...
			final String preposition = tokens.nextToken (getWsPunc ());
			if ("in".equals (preposition)) {
				...
				throw new GameLogicException (
						"Don't understand in what? " + inWhat, tokens,
						this);
			}
			...
			if ("for".equals (preposition) || "by".equals (preposition)
			...
			if ("then".equals (preposition)
					|| "do".equals (preposition)) return clause;
			throw new GameLogicException (
					"Don't understand test preposition? " + preposition,
					tokens, this);
		}
	}
