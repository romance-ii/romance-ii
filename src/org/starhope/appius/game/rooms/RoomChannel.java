...
public class RoomChannel extends Channel {
	 ...
	 */
	final private List <RoomListener> listeners = Collections.synchronizedList (new LinkedList <RoomListener> ());
	 ...
	 */
	public RoomChannel (final String name, final Zone newZone, final Room newRoom) {
		super (name, newZone);
		room = newRoom;
	}
	 ...
	 */
	public void broadcastAcceptGameAction (final AbstractUser u, final JSONObject jso) {
	 ...
	 */
	private void broadcastAcceptObjectJoinChannel (final RoomListener joiner) {
		for (final RoomListener listener : getAllListeners ()) {
	 ...
	 * @param user WRITEME ewinkelman@resinteractive.com
	 */
	private void broadcastAcceptObjectPartChannel (final RoomListener parter) {
		for (final RoomListener listener : getAllListeners ()) {
	 ...
	 * @param user WRITEME ewinkelman@resinteractive.com
	 */
	public void broadcastAcceptOutOfBandMessage (final AbstractUser sender, final JSONObject body) {
		for (final RoomListener listener : getAllListeners ()) {
	 ...
	 */
	public void
			broadcastAcceptUserVariableUpdate (final AbstractUser user, final String varName, final String varValue) {
		for (final RoomListener listener : getAllListeners ()) {
			listener.acceptUserVariableUpdate (user, varName, varValue);
		}
	}
	...
	/**
	 * WRITEME: Document this method WRITEME ewinkelman@resinteractive.com
	 * 
	 * @return WRITEME ewinkelman@resinteractive.com
	 */
	public Room getRoom () {
	 ...
	 * @return WRITEME ewinkelman@resinteractive.com
	 */
	public boolean join (final RoomListener listener, final boolean suppressBroadcast) {
	 ...
	 * @return WRITEME ewinkelman@resinteractive.com
	 */
	public boolean part (final RoomListener listener, final boolean suppressBroadcast) {
		listeners.remove (listener);
