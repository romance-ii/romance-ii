...
public class OperatorControlClick implements RunCommands {
    ...
    static final class ClickHandler implements ActionMethod {
         ...
         */
        final private SecurityCapability capSysOp =
        ...
        public boolean acceptAction (
            ...
            if (null != indirectObject
                ...
                OperatorControlClick.opClick (subject,
                        where, target, indirectObject);
            }
            return true;
        }
    }
    ...
    protected static void opClick (final AbstractUser operator,
		...
		if (null != userClicked) {
            operator.acceptMessage (userClicked.getAvatarLabel (),
                    "Nomenclator", OpCommands
                    .fingerInfo (operator, userClicked));
            operator.acceptPrivateMessage (userClicked,
                    "$$: My name is “" + userClicked.getAvatarLabel ()
                    + "”");
            try {
                final JSONObject promptData = new JSONObject ();
                promptData.put ("id", "^click+"
                        + userClicked.getUserID ());
				promptData.put ("attachUser", clickedOn);
                promptData.put ("label", "Nomenclator");
                promptData.put ("label_en_US", "Nomenclator");
                promptData.put ("title", userClicked.getAvatarLabel ());
                promptData
                .put ("msg", "What do you want to do to him?");
                ...
                operator
                .acceptSuccessReply ("prompt", promptData, room);
    ...
    public void run () {
        ...
        Quaestor.listen (new ActionHandler (null, null, "click", null,
                new ClickHandler ()));
