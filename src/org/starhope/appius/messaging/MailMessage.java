 ...
 * @author twheys@gmail.com Timothy W. Heys Timothy W. Heys
	...
	/**
	 * @param originalMessage the message ID to which replies are
	 *             wanted
	 ...
	 * created the flag and the value of the flag names. See MySQL
	 * table messageFlags.
	 */
	...
	public void addFlag (final int fromUserID, final MessageFlag flag) {
		...
		try {
			...
			st = con.prepareStatement ("INSERT INTO messageFlags (postID, userID, flag) VALUES (?, ?, ?)");
			st.setInt (1, getID ());
	...
	public Vector <MailMessage> getRepliesOnWall () {
		final Vector <MailMessage> messages = new Vector <MailMessage> ();
		Connection con = null;
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("SELECT ID FROM messages WHERE isDeleted='W' AND inReplyTo=? AND body<>'' ORDER BY messages.sentTime ASC");
			...
			while (rs.next ()) {
				...
				try {
					messages.add ( ((MailMessageSQLLoader) m
							.getRecordLoader ()).set (m,
							rs.getInt (1)));
				} catch (final NotFoundException e) {
					BugReporter
							.getReporter ("srv")
							.reportBug (
									"Caught a NotFoundException in MailMessageSQLLoader.getMailOnWallInReplyTo ",
									e);
				}
			}
		} catch (final SQLException e) {
