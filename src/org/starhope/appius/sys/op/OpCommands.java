...
public class OpCommands {
	...
	private static String assertValidWarnReason (
		...
		if (LibMisc.getText ("rule." + reason) == null) {
			u.acceptMessage ("Invalid Warning Reason Code",
					"Catullus", "“" + warnReason
	...
	public static void exec (final Channel channel,
			final AbstractUser u, final String commandString) {
		...
		try {
			opCommandHandler = OpCommands.class.getMethod ("op_"
					+ cmd, String [].class, AbstractUser.class,
					Channel.class);
			...
			try {
				opCommandHandler = LibMisc.loadExtension (
						OpCommands.class).getMethod ("op_" + cmd,
						String [].class, AbstractUser.class,
						Channel.class);
				...
				final NetIOThread thread = u.getServerThread ();
				if (null != thread) {
					thread.sendError_RAW ("*", "No such method: "
							+ cmd);
				}
				return;
		...
		try {
			...
			if (words.length > 1) {
				subcommand = Arrays.copyOfRange (words, 1,
						words.length);
			} else {
			...
			BugReporter.getReporter ("srv").reportBug (
					"Exception while handling xtn command " + cmd,
					e.getCause ());
			OpCommands.sendUserErrorMessage (u, cmd, e.getCause ());
			final NetIOThread thread = u.getServerThread ();
			if (null != thread) {
				thread.sendError_RAW (cmd, e.getCause ()
						.toString ());
			}
			u.sendOops ();
			return;
	...
	private static String getConfKey (final String string) {
		if (2 < string.length ()) {
			if ('~' == string.charAt (0)) {
				...
				case 'a':
					return "org.starhope.appius"
							+ string.substring (2);
				case 't':
					return "com.tootsville" + string.substring (2);
				case 'e':
					return "com.empiresoftheair"
							+ string.substring (2);
				}
			}
		}
	...
	/**
	 * Gets a list of rooms for the user
	 * 
	 * @param roomsString WRITEME 
	 * @param u WRITEME 
	 * @return WRITEME 
	 */
	private static LinkedList <Room> getRoomList (
			final String roomsString, final AbstractUser u) {
		final LinkedList <Room> rooms = new LinkedList <Room> ();
		if ( (roomsString == null) || "".equals (roomsString)) {
			rooms.add (u.getRoom ());
		} else if (roomsString.startsWith ("#all")) {
			rooms.addAll (u.getZone ().getRoomList ());
		} else {
			final String [] roomList = roomsString.split (",");
			for (final String roomMoniker : roomList) {
				final Room room = u.getZone ().getRoom (
						roomMoniker.trim (), null);
				if (room != null) {
					rooms.add (room);
				}
			}
	 ...
	 * non-staff members, give a total of public users, and the ratio
	 * of paid vs. free members.
	 * 
	 * @param u The user requesting this headcount
	 * @param channel The room in which that user is standing.
	 */
	...
	private static void headcount_rooms (final AbstractUser u,
		...
		while (numbers.hasNext ()) {
			...
			try {
				for (final AbstractUser userInZone : AppiusClaudiusCaecus
					...
					rooms.put (userCurrentRoom, Integer
							.valueOf (1 + knew.intValue ()));
					++grandTotal;
				}
			} catch (final NotFoundException e) {
				BugReporter.getReporter ("srv").reportBug (
		...
		headcount.append ("\n\n");
		u.acceptMessage ("Headcount for all Rooms", "Census",
				headcount.toString ());
	}
	...
	public static void hook (final Channel channel,
			final AbstractUser u, final String speech) {
		final String command = speech.substring (1,
				speech.indexOf (' ') - 1);
		...
		try {
			...
			u.acceptMessage ("$@ $" + command, "Catullus",
					LibMisc.stringify (e));
		}
	}
	...
	public static void op_ban (final String [] words,
		...
		if ( !u.hasStaffLevel (brat.getStaffLevel () + 1)) {
			u.acceptMessage (
					...
					"You can not ban: “"
							+ bratLogin
							+ "” because they are a staff member.");
			return;
		}
		
		brat.ban (u, banReason);
		u.acceptMessage ("#ban", "Catullus",
				"Banned user " + brat.getAvatarLabel () + ": "
	 ...
	 * </p>
	 * <p>
	 * Bans can be lifted using #banhammer #-ip IP-ADDRESS (or
	 * hostname)
	 * </p>
	 * <p>
	 * A ban can be placed with #banhammer #+ip IP-ADDRESS or
	 * #banhammer #+ip HOSTNAME or #banhammer #user USERNAME. In the
	 * latter case, the user's connected IP address is used. This is
	...
	public static void op_banhammer (final String [] words,
		...
		if ("#list".equals (targetType)) {
			...
			u.acceptMessage ("#banhammer #list", "DJBlakkat",
					list.toString ());
		}
	...
	/**
	 * <p>
	 * Moves yourself or another user into another room. Must have
	 * staff level 8 (DEVELOPER) to use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	 * <code>#beam [LOGIN] [ROOM]</code>
	 * <p>
	 * <p>
	 * <strong>Examples</strong><br/>
	 * <code>#beam tootSquareWest</code>
	 * <code>#beam mouser tootSquareWest</code>
	 * </p>
	 * 
	 * @param words The command parameters (whitespace-delimited list)
	 *             provided after the # command name
	...
	public static void op_beam (final String [] words,
		...
		if ( (words.length < 1) || (words.length > 2)) {
			u.acceptMessage (
					"#beam",
					"Tootsville",
					"#beam accepts one or two parameters of a destination room or a target and destination room");
		}
		...
		final AbstractUser targetUser = words.length == 1 ? u
				: Nomenclator.getUserByLogin (words [0]);
		if (null == targetUser) {
			u.acceptMessage ("#beam", "Tootsville",
					"Unable to find user " + words [0]);
			return;
		}
		
		final String roomString = words.length == 1 ? words [0]
				: words [1];
		final Room destination = u.getZone ().getRoom (roomString,
				null);
		if (destination == null) {
			u.acceptMessage ("#beam", "Tootsville",
					"Unable to find destination " + words [0]);
			return;
		}
		
		final JSONObject greenBabe = new JSONObject ();
		greenBabe.put ("room", roomString);
		targetUser.doTransport ();
		targetUser.acceptSuccessReply ("beam", greenBabe,
				u.getRoom ());
	}
	
	/**
	 * Simply reference a range of users, for testing purposes. Takes
	 * an optional low and high point, or runs 0…250000. (250,000) This
		...
		for (int i = high; i > low; i-- ) {
			...
			if ( (i % 10000) == 0) {
				u.acceptMessage ("Census", "Census",
						"Census has reached " + (i / 1000)
								+ " thousand");
			}
			if (null != someone) {
				someone.acceptMessage ("Welcome", "Welcome",
						"Welcome");
			}
		}
	}
	...
	/**
	 * <p>
	 * Clear a GameEvent from a Zone. Must have staff level 4
	...
	public static void op_clearevent (final String [] words,
		...
		for (final GameBase ev : z.getGameEvents ()) {
	...
	public static void op_clearvar (final String [] words,
		...
		if ( (words.length < 1) && (words.length > 2)) {
			u.acceptMessage (
					"#clearvar",
					"Tootsville",
					"#setvar only accepts one parameter with an optional location parameter of a list of comma delimited rooms or #all");
		}
		
		final LinkedList <Room> rooms = OpCommands.getRoomList (
				words.length == 2 ? words [0] : "", u);
		final String varName = words.length == 1 ? words [0]
				: words [1];
	...
	public static void op_createroom (final String [] words,
		...
		u.assertStaffLevel (User.STAFF_LEVEL_DESIGNER);
		final Room r = Nomenclator.make (Room.class);
	...
	public static void op_dropkick (final String [] words,
		...
		for (int i = 0; i < 3; ++i) {
			...
			// OpCommands.log.debug ("dropkick");
			if (null == thread) {
				u.acceptMessage ("dropkick", "Catullus",
						brat.getAvatarLabel () + " in " + i
								+ " tries");
				return;
			}
			thread.close ();
	...
	public static void op_dumpthreads (final String [] words,
		...
		final File runDir = new File (
				AppiusConfig.getConfigOrDefault (
						"org.starhope.appius.runDir",
						"/opt/appius/run/"));
		final File dump;
		...
		try {
			...
			for (final Zone zone : AppiusClaudiusCaecus
				...
				for (final Room aRoom : zone.getRoomList ()) {
					...
					out.write (aRoom.getFullMoniker ().getBytes ());
					out.write ('\n');
			...
			try {
				out.write (Nomenclator.getAllCacheInfo ()
						.getBytes ());
			...
			out.write ("\n\n ---> Command execution profiling:\n\n"
					.getBytes ());
			out.write (CommandExecutorThread.dumpTimes ()
					.toString ().getBytes ());
			out.write ("\nEND\n".getBytes ());
	...
	public static void op_echojson (final String [] words,
		...
		final JSONObject jso = new JSONObject (s.toString ());
		try {
	...
	public static void op_finger (final String [] words,
		...
		u.acceptMessage (pasivo.getAvatarLabel (), "Finger",
				OpCommands.fingerInfo (u, pasivo));
		return;
	}
	
	/**
	 * Attempt to flush the pending database records to the database
	 * (if any). Reports back how many remain pending after the sweep.
	 * Does not affect the ongoing background sweep process, which will
	...
	public static void op_flush (final String [] words,
		...
		final int left = DataRecordFlushManager
				.flush (words.length > 0 ? Integer
						.parseInt (words [0]) : 100);
		u.acceptMessage ("Flush", "DataRecordFlushManager",
				"Flushed some records. " + left
						+ " remain pending.");
	}
	...
	public static void op_getconfig (final String [] words,
		...
		try {
			u.acceptMessage ("Configuration: " + confKey,
					"Catullus", AppiusConfig.getConfig (confKey));
		} catch (final NotFoundException e) {
			u.acceptMessage ("Configuration: " + confKey,
					"Catullus", "(not set)");
		}
	...
	public static void op_getevents (final String [] words,
		...
		final StringBuilder list = new StringBuilder ();
		for (final GameBase ev : u.getZone ().getGameEvents ()) {
	...
	public static void op_getschedule (final String [] words,
			...
			throws PrivilegeRequiredException {
		u.assertStaffLevel (User.STAFF_LEVEL_MODERATOR);
		final Map <Long, Runnable> schedule = AppiusClaudiusCaecus
		...
		if (schedule.size () == 0) {
			...
			for (final Map.Entry <Long, Runnable> activity : schedule
					.entrySet ()) {
				s.append ('\n');
	...
	public static void op_getschedulefor (final String [] words,
		...
		final StringBuilder s = new StringBuilder (words [0]);
		if (schedule.size () == 0) {
			s.append ("(no scheduled activities)");
		} else {
			for (final Map.Entry <Long, Runnable> activity : schedule
		...
		u.acceptMessage ("Schedule for " + words [0], "Kalendor",
				s.toString ());
	}
	...
	public static void op_getuvar (final String [] words,
		...
		final AbstractUser whom = Nomenclator
		...
		final String uvar = whom.getVariables ().get (variable);
	...
	public static void op_getuvars (final String [] words,
		...
		final StringBuilder gotVars = new StringBuilder (
				whom.getAvatarLabel ());
		gotVars.append ("\n\n");
		for (final Entry <String, String> var : whom.getVariables ()
				.entrySet ()) {
			gotVars.append ('“');
	...
	private static void op_getvar (final String [] words,
		...
		if (words [0].startsWith ("@")) {
			final Room thatRoom = u.getZone ().getRoom (
					words [0].substring (1), null);
			if (null != thatRoom) {
	...
	/**
	 * <p>
	 * Get all room variables. Must have staff level 4 (DESIGNER) to
	 * use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	...
	private static void op_getvars (final String [] words,
		...
		if (words.length > 0) {
			final Room where = room
					.getZone ()
					.getRoom (words [0].charAt (0) == '@' ? words [0].substring (1)
							: words [0], null);
			if (room != null) {
		...
		u.acceptMessage (room.getTitle (), "Catullus",
				gotVars.toString ());
	}
	...
	public static void op_grant (final String [] words,
			...
			throws PrivilegeRequiredException {
		u.assertStaffLevel (User.STAFF_LEVEL_STAFF_MEMBER);
		final String itemNumber = words [1];
		final String taker = words [0];
		final int itemCount = words.length > 2 ? Integer.valueOf (
				words [2]).intValue () : 1;
		
		final int itemNumberInt = Integer.parseInt (itemNumber);
		
		final AbstractUser pasivo = Nomenclator
				.getUserByLogin (taker);
		if (null == pasivo) {
			OpCommands.sendAdminMessage (channel, u,
					"Can't find user: “" + taker + "”");
			return;
		}
		final UserInventory pasivoInventory = pasivo.getInventory ();
		RealItem newItem;
		try {
			newItem = Nomenclator.getDataRecord (RealItem.class,
					itemNumberInt);
			pasivoInventory.addItem (newItem, itemCount);
		} catch (final NotFoundException e) {
			OpCommands.sendAdminMessage (channel, u,
					"Can't find item: “" + itemNumberInt + "”");
			return;
		}
		...
		u.acceptMessage ("Gift given", "Lifeguard",
				message.toString ());
		final ADPEarning earnings = new ADPEarning (u);
		earnings.setUser (pasivo);
		earnings.setItem (newItem, itemCount);
		pasivo.getRoom ().getRoomChannel ().broadcast (earnings);
		return;
	}
	 ...
	 * @param channel WRITEME
	 */
	public static void op_https (final String [] words,
			final AbstractUser u, final Channel channel) {
		// Check preconditions
		try {
			u.assertStaffLevel (User.STAFF_LEVEL_DESIGNER);
		...
		if (words.length != 1) {
			u.acceptMessage ("#https", "Tootsville",
					"#https expects either 'start', 'stop', or 'restart'");
			return;
		}
		
		if ("start".equals (words [0])) {
			if ( !AppiusClaudiusCaecus.getHttps ().running ()) {
				AppiusClaudiusCaecus.getHttps ().start ();
			}
		} else if ("stop".equals (words [0])) {
			if (AppiusClaudiusCaecus.getHttps ().running ()) {
				AppiusClaudiusCaecus.getHttps ().shutdown ();
			}
		} else if ("restart".equals (words [0])) {
			if (AppiusClaudiusCaecus.getHttps ().running ()) {
				AppiusClaudiusCaecus.getHttps ().shutdown ();
			}
			try {
	 ...
	 *             types, refer to
	 *             {@link Commands#do_getInventoryByType(JSONObject, AbstractUser, Room)}
	 *             — note that this supports only a single item type
	 *             (or type-set)
	...
	public static void op_inv (final String [] words,
		...
		final StringBuilder invMsg = new StringBuilder ();
		final AbstractUser who = Nomenclator
				.getUserByLogin (words [0]);
		
		for (final Entry <RealItem, Integer> entry : who
				.getInventory ().getItems ().entrySet ()) {
			final RealItem realItem = entry.getKey ();
			invMsg.append (realItem.getRealID ());
			invMsg.append (" - ");
			invMsg.append (realItem.getItemReference ().getTitle ());
			invMsg.append (" (");
			invMsg.append (entry.getValue ());
			invMsg.append (")");
			invMsg.append ('\n');
		}
		u.acceptMessage ("Inventory of " + who.getDebugName (),
				"Tootsville", invMsg.toString ());
	}
	
	/**
	 * Kick a user offline for a certain reason
	 * <p>
	 * Kick a user offline for a certain reason. Must have staff level
	 * 2 (MODERATOR) to use this command.
	 * </p>
	 * <p>
	 ...
	 * <li>APP.PARN = You need your parent's permission in order to
	 * chat in Tootsville.</li>
	 * <li>APP.MAIL = You need to confirm your eMail address in order
	 * to chat in Tootsville.</li>
	 ...
	 * @param u The user invoking the operator command
	 * @param channel The channel in which the user is in
	 * @throws NotFoundException if the warning reason code is not
	 *              valid
	 */
	...
	public static void op_loadlists (final String [] words,
		...
		// log.debug ("(re-)loading censorship for "
		// + u.getZone ().getName ());
		try {
			AppiusConfig.getFilter (FilterType.KID_CHAT)
					.reloadLists (
							AppiusConfig
	 ...
	 * @param channel WRITEME 
	 */
	public static void op_loadvar (final String [] words,
			final AbstractUser u, final Channel channel) {
		// Check preconditions
		try {
			u.assertStaffLevel (User.STAFF_LEVEL_DESIGNER);
		...
		if (words.length > 1) {
			u.acceptMessage (
					"#loadvar",
					"Tootsville",
					"#loadvar only accepts one optional paramter of a list of comma delimited rooms or #all");
		}
		
		// Assemble the list of rooms to be affected
		final LinkedList <Room> rooms = new LinkedList <Room> ();
		if (words.length == 0) {
			rooms.add (u.getRoom ());
			...
			final String [] roomList = words [0].split (",");
			for (final String roomMoniker : roomList) {
				final Room room = u.getZone ().getRoom (
						roomMoniker.trim (), null);
				if (room != null) {
	...
	/**
	 * <p>
	 * Display some memory usage and other debugging type information
	 * as an pop-up message. Must have Designer privileges to use this
	 * command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	...
	/**
	 * <p>
	 * Display information about or micromanage the metronome. Must
	 * have staff level 8 (DEVELOPER) to use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	...
	/**
	 * <p>
	 * Set the message of the day. Must have staff level 4 (DESIGNER)
	 * to use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	...
	public static void op_nuke (final String [] words,
			...
			throws PrivilegeRequiredException {
		u.assertStaffLevel (User.STAFF_LEVEL_DEVELOPER);
		final Room hiroshima = u.getZone ().getRoom (words [0], null);
		if (hiroshima == null) {
			u.acceptMessage ("Target acquisition failed",
					"Enola Gay",
					"Sir, we can't identify the target “"
							+ words [0] + "”");
			return;
		}
		for (final AbstractUser victim : hiroshima.getAllUsers ()) {
			final ServerThread thread = victim.getServerThread ();
			if (null != thread) {
	 ...
	 * name — or can also be the special value of “ <tt>#me</tt>” if
	 * you are signed in as the avatar in question.</li>
	 * <li>“where” can be the moniker of a room, or the special value
	 * of “<tt>#here</tt>” to mean the room in which you are currently
	 * located.</li>
	 * </ul>
	 */
	...
	public static void op_parentapproves (final String [] words,
		...
		final AbstractUser kid = Nomenclator.getUserByLogin (login);
		if (null == kid) {
			u.acceptMessage ("#parentapproves", "Catullus",
		...
		if ( (words.length > 2) && words [2].equals ("#force")) {
			...
			u.acceptMessage (
					"#parentapproves",
					"Catullus",
					"Reset kid's parent from “"
							+ kidsParent.getMail () + "” to “"
							+ parentMail
	...
	/**
	 * <p>
	 * Ping the server, to force a neutral administrative message
	 * reply. Must have staff level 1 (STAFF) to use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	 * <code>#ping</code>
	 ...
	 * event Places, and adds them to the room in the given WHERE
	 * place. WHERE can be a diamond-shaped area around the operator
	 * issuing the command (using #here, #here-tiny, or #here-big), or
	 * can be an explicitly-issued polygon string. The event region ID
	...
	public static void op_place (final String [] words,
		...
		if ("#item2".equals (what)) {
			room.setVariable (placeZone, "evt_itm2_" + words [2]
					+ "_" + words [3] + ":" + where);
			return;
		}
		if ("#item2".equals (what)) {
	 ...
	 * (“run”) method and the “new zone run commands” (“newZone”)
	...
	/**
	 * <p>
	 * Forces a zone to retire. This will disconnect anyone currently
	 * in the zone. Use #evacuate to move users to another zone. Must
	 * have staff level 8 (DEVELOPER) to use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	...
	/**
	 * <p>
	 * Set a config property. Must have staff level 8 (DEVELOPER) to
	 * use this command.
	 * </p>
	 * <p>
	 * <strong>Syntax for use</strong><br/>
	...
	public static void op_setstafflevel (final String [] words,
		...
		final AbstractUser guy = Nomenclator
				.getUserByLogin (words [1]);
	...
	public static void op_setvar (final String [] words,
		...
		if ( (words.length < 2) || (words.length > 3)) {
			u.acceptMessage (
					"#setvar",
					"Tootsville",
					"#setvar only accepts two parameters with an optional location parameter of a list of comma delimited rooms or #all");
		}
	...
	public static void op_speak (final String [] words,
		...
		if (null == brat) {
			u.acceptMessage ("#speak", "Catullus",
					"Can't find user: “" + bratLogin + "”");
			return;
		}
		...
		brat.acceptMessage (
				"",
				"Lifeguard",
				LibMisc.getTextOrDefault ("chat.enabled",
	...
	public static void op_stfu (final String [] words,
		...
		if (null == brat) {
			u.acceptMessage ("#stfu", "Catullus",
					"Can't find user: “" + bratLogin + "”");
			return;
		}
		
		brat.setCanTalk (false);
		brat.acceptMessage ("", "Lifeguard", LibMisc
				.getTextOrDefault ("chat.disable",
						"You are no longer allowed to chat."));
		u.acceptMessage ("", u.getAvatarLabel (), bratLogin
	...
	public static void op_testcensor (final String [] words,
		...
		final FilterResult result = AppiusConfig.getFilter (
				FilterType.KID_CHAT).filterMessage (message);
		u.acceptMessage (
				"#testcensor",
				"Catullus",
				String.format (
	...
	public static void op_time (final String [] words,
		...
		u.acceptMessage ("#time", "Catullus",
				"The time is now: " + System.currentTimeMillis ()
						+ " or " + Messages.prettyDate (today)
						+ " at " + now.toString ());
		return;
	}
	...
	public static void op_v (final String [] words,
		...
		if ('@' == atParam.charAt (0)) {
			...
			puppetRoom = u.getZone ().getRoom (roomName, null);
			if (null == puppetRoom) {
	...
	public static void op_wall (final String [] words,
		...
		for (final AbstractUser anybody : u.getZone ()
				.getAllUsersInZone ()) {
			anybody.acceptMessage (
					"Important Message",
					anybody.hasStaffLevel (User.STAFF_LEVEL_STAFF_MEMBER) ? u
							.getAvatarLabel () : "ADMIN",
					speechString);
		}
		return;
	}
	...
	public static void op_wallzones (final String [] words,
		...
		for (final AbstractUser anybody : AppiusClaudiusCaecus
			...
			if (anybody.getStaffLevel () >= u.getStaffLevel ()) {
				anybody.acceptMessage ("Write to All in all Zones",
						u.getAvatarLabel (), speech.toString ());
			} else {
	...
	public static void op_warn (final String [] words,
		...
		if (null == brat) {
			u.acceptMessage ("#warn", "Catullus",
					"Can't find user: “" + bratLogin + "”");
			return;
		}
		final String warning = String.format (
				LibMisc.getText ("rule." + warnReason),
				brat.getAvatarLabel ());
		if (brat.isOnline ()) {
		...
		u.acceptMessage ("@" + brat.getAvatarLabel (),
				u.getAvatarLabel (), warning);
	}
	 ...
	 * <li>@[ROOM] for the location of every user in the specified
	 * room.</li>
	 * </ul>
	 * <p>
	 * <strong>Examples</strong><br/>
	...
	public static void op_whereis (final String [] words,
		...
		if (whoWeWant.charAt (0) == '@') {
			OpCommands.whereis_atRoom (u, channel,
		...
		if (null == place) {
			if (them.isNPC ()) {
				try {
					final UserRecord rec = Nomenclator
							.getDataRecord (UserRecord.class,
									whoWeWant);
					final Set <WeakReference <AbstractUser>> npcs = Nomenclator
							.getOnlineNPCs (rec.getLogin ()
									.toLowerCase (
											Locale.ENGLISH));
					final StringBuilder s = new StringBuilder ();
					for (final WeakReference <AbstractUser> npc : npcs) {
						final AbstractUser n = npc.get ();
						if (null == n) {
	...
	public static void op_who (final String [] words,
		...
		Room theRoom;
		if (words.length >= 1) {
			theRoom = u.getZone ().getRoom (words [0], null);
		} else {
			theRoom = u.getRoom ();
		}
		...
		u.acceptMessage (theRoom.getName (), "Catullus",
				roster.toString ());
	}
	...
	public static void op_zoom (final String [] words,
		...
		// UserTransients.getEffects (user).heightScalar =
	...
	private static void sendUserErrorMessage (final AbstractUser u,
		...
		if ( (null != userThread)
				&& ((NetIOThread) userThread)
						.getVerboseBugReplies ()) {
			u.acceptMessage ("$@", "Java",
	...
	private static void setvar_replace (final String [] words,
		...
		if (changed.size () > 0) {
			msg.append (": ");
			msg.append (LibMisc.listToDisplay (changed,
					u.getLanguage (), u.getDialect ()));
		}
		u.acceptMessage ("#setvar #replace", "Catullus",
				msg.toString ());
		return;
	}
	...
	private static void whereis_atRoom (final AbstractUser u,
		...
		for (final AbstractUser them : u.getZone ()
				.getRoom (roomMoniker, null).getAllUsers ()) {
			report.append (them.getAvatarLabel ());
			report.append ('\t');
			report.append (them.getLocation ().toString ());
			report.append ('\n');
	...
	private static void whereis_everyone (final AbstractUser u,
		...
		for (final Integer theirID : u.getZone ()
			...
			if (null == place) {
				...
				if (null == roomIpse) {
					report.append ("Nomenclator's user thinks room is null, but zone's copy of user thinks it's "
							+ place.getTitle () + "\n");
				} else if ( !roomIpse.getFullMoniker ().equals (
						place.getFullMoniker ())) {
					report.append (" !mismatch: zone's user is in "
							+ place.getTitle ()
							+ " but Nomenclator version is in "
							+ roomIpse.getFullMoniker () + "\n");
				}
			}
		}
		u.acceptMessage ("Where is everyone?", "Catullus",
				report.toString ());
		return;
	}
	...
	public static void z$z (final AbstractUser u) {
		u.acceptMessage (
				"By...",
				"Star-Hope",
				LibMisc.rot13 ("Nccvhf Pynhqvhf Pæphf, Tnvhf Inyrevhf Pnghyyhf, Pybqvn Zrgryyv Chypure, Choyvhf Iretvyvhf Zneb, naq eryngrq grpuabybtvrf hfrq va gur perngvba bs guvf tnzr ner Pbclevtug © 1998-2010, Oehpr-Eboreg Cbpbpx, naq ner yvprafrq haqre gur grezf bs gur TAH Trareny Choyvp Yvprafr (TCY) I3. uggc://jjj.Fgne-Ubcr.Bet/"));
		try {
			for (final String msg : (String []) AppiusConfig
					.getUserClass ().getMethod ("getZ$Z")
					.invoke (null, (Object []) null)) {
				u.acceptMessage ("By...", "Catullus",
						LibMisc.rot13 (msg));
			}
		} catch (final Throwable t) {
			/* No Op */
