...
public abstract class SQLPeerList <T extends SQLPeerListElement>
	...
	/**
	 * WRITEME: document this field (brpocock@star-hope.org, Jan 12,
	 * 2010) cache (SQLPeerList)
	 */
	...
	protected SQLPeerList (final int cacheSize, final String cacheName) {
		...
		try {
			cache = (Cache <Long, T>) cacheManager
	...
	/**
	 * WRITEME: document this method (brpocock@star-hope.org, Jan 12,
	 * 2010)
	 */
	...
	public boolean contains (final Object o) {
		...
		try {
			final T element = (T) o;
			T found = cache.retrieve (Long.valueOf (element
					.getSQLPeerID ()));
			if (null != found) {
				found = fetch (element);
				return null != found;
	 ...
	 * @return a prepared statement to insert a given item into the
	 *         list. The item will be added to the statement by setting
	 *         its ID as the first parameter.
	 */
