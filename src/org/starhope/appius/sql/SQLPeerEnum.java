...
public abstract class SQLPeerEnum extends SQLPeerDatum {
	...
	/**
	 * WRITEME: document this field (brpocock@star-hope.org, Oct 13, 2009)
	 ...
	 * @return WRITEME
	 */
	public static <T extends SQLPeerEnum> T get (final Class <T> klass,
			final int id) {
		SQLPeerEnum o = null;
		try {
			o = klass.newInstance ();
	...
	/**
	 * Deprecated from birth: backward-compatibility routine for objects
	 * that have been migrated from SQLPeerEnum to DataRecord
	 * 
	 * @param <T> class
	 * @param klass class
	 * @param id ID
	 ...
	 * object. The ResultSet must have a long (probably INT UNSIGNED) in
	 * the first column of the results, and the string value for it in
	 * column 1.
	 * 
	...
	public boolean equals (final Object o) {
		if (o instanceof SQLPeerEnum) return equals ((SQLPeerEnum) o);
		return false;
	}
	...
	public int getID (final String s) {
		for (final Entry <Integer, String> e : getEnumeration ()
				.entrySet ()) {
			if (e.getValue () == s) return e.getKey ().intValue ();
		}
		return -1;
	}
	...
	protected synchronized void prepCache () {
		...
		if (cacheLevel >= 0) // + getClass ().getCanonicalName () + "#*#");
			return;
		Connection con = null;
		...
		try {
			...
			try {
				...
				} else throw AppiusClaudiusCaecus.fatalBug (new Exception (
		...
		SQLPeerEnum.hasCached.put (this.getClass (), Integer
				.valueOf (1));
