...
public final class CollisionObject implements Collidable {
	 ...
	 */
	public CollisionObject (final Collidable obj, final long currentTime) {
		mass = obj.getMass ();
		collisionBounds = obj.getCollisionBounds ();
		centerofMass = obj.getCenterOfMass ();
		final double x = centerofMass.getX ();
		if (Double.isNaN (x)) {
			AppiusClaudiusCaecus.reportBug ("Hate you");
		}
    ...
    /**
     * Moves the object forward/backwards by x milliseconds Also watches
     * for objects that start/stop
     *
    ...
    public void move (final long milliseconds) {
        ...
        if (moveEndTime > moveStartTime) { // Normal case
            if (referenceTime < moveStartTime) {
                ...
                ms = temp < moveEndTime ? referenceTime - moveEndTime
                        : 0;
            } else {
                ms = temp > moveEndTime ? moveEndTime - referenceTime
                        : ms;
                ms = temp < moveStartTime ? moveStartTime
                        - referenceTime : ms;
            }
        } else if (moveEndTime < moveStartTime) { // Highly unusual. Not
                                                  // handling the case
                                                  // of starting
            ...
            if (referenceTime > moveStartTime) {
                ...
                ms = temp > moveStartTime ? temp - moveStartTime : 0;
