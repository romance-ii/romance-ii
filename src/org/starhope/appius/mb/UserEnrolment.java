 ...
 * time.
 *
 * @author brpocock@star-hope.org, <a href="mailto:twheys@gmail.com">Tim
 *         Heys</a>
 */
public class UserEnrolment extends SimpleDataRecord <UserEnrolment>
	...
	/**
	 * A timestamp far enough in the past to be sure to have passed before this could be created. This
	 * value is 2007-01-01 at midnight, and no current Appius implementor had any members before that
	 * time.
	 */
	...
	private static String genRandomOrderCode () {
		...
		for (int i = 0; i < 10; ++i) {
			code.append (String.valueOf (UserEnrolment
					...
					0, UserEnrolment.getOrderCodeChars ().length - 1)]));
		}
				...
				while (rs.next ()) {
					...
					try {
						enrolment = Nomenclator.getDataRecord (
								UserEnrolment.class, rs.getInt ("ID"));
	 ...
	 * </p>
	 * <p>
	 * Note: this is the ctor called from {@link MBSession}.payAccount()
	 * </p>
	 *
	...
	public UserEnrolment (final String order_source,
		...
		try {
			...
			AppiusClaudiusCaecus.reportBug ("Cannot create enrolment",
					e1);
		}
	}

	/**
	 * Create a new user enrolment with the specified order source and order code. Dates and enrolment type options
	 * must be set later.
	 *
	 ...
	 * activatedUser.isPaidMember () instead of requiring a boolean into
	 * it
	 * </p>
	 *
	...
	public void activate (final boolean newEnrolment) {
		if (isActivated) {
			AppiusClaudiusCaecus
					.reportBug ("Called UserEnrolment.activate twice in a row");
			return;
		}
		if (userID > 1) {
			if (newEnrolment) {
				...
				if (activatedUser instanceof GeneralUser) {
					...
					AppiusClaudiusCaecus
							.reportBug ("Unhandled case... non-local user got enrolment...");
				}
				startEnrolment ();
			} else {
				continueEnrolment ();
			...
			AppiusClaudiusCaecus
					.reportBug ("Trying to create an enrolment for User #1");
		}
		isActivated = true;
	}
	 ...
	 * based upon the calendar day; i.e. we take into account the length
	 * differences of months and so forth; then, additional days are
	 * appended.
	 * </p>
	 * <p>
	 * Out of paranoia, the base date (to which the new months & days
	 * are added) will be set to the <em>later</em> of <em> either </em>
	 * the current expiry date, or the current date. This means that if
	 * an enrolment expires in the past, and is then extended, it will
	 * end at the proper date from now; but if it has not yet expired,
	 * the renewal will be tacked onto the end.
	 * </p>
	 *
	...
	private void generateOrderCode () {
		...
		while (true) {
			...
			try {
				UserEnrolment.getBySourceAndCode (orderSource, tryCode);
			} catch (final NotFoundException e) {
				break;
			}
		}
	...
	private java.sql.Date getLastExpiration () {
		...
		for (final UserEnrolment enrolment : UserEnrolment
			...
			if (lastExpiration.before (enrolment.getExpires ())) {
				AppiusClaudiusCaecus.blather ("Unit Test ("
		...
		AppiusClaudiusCaecus
				.blather ("Unit Test (Last Expiration for User ID#"
	...
	public void setAuthSubID (final String authSubID2)
		...
		if (null == authSubID2) {
			AppiusClaudiusCaecus
					.reportBug ("ARB failing. subscription id: "
							+ getID () + " order code: " + orderSource
							+ "-" + orderCode);
			authSubID = null;
		} else {
	...
	private void startEnrolment () {
		...
		if (beginsOn.getTime () < 1289933972 /* Thu 16 Nov 2010, 2pm */) {
			AppiusClaudiusCaecus
					.reportBug ("Enrolment start date incorrect: "
							+ beginsOn.toString ());
		}
		try {
			setBegins (beginsOn);
	...
	public String toString () {
		return "UserEnrolment [authSubID=" + authSubID + ", begins="
				+ begins + ", expires=" + expires + ", id=" + getID ()
				+ ", isActivated=" + isActivated + ", orderCode="
				+ orderCode + ", orderSource=" + orderSource
