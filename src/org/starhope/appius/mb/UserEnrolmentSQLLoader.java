...
public class UserEnrolmentSQLLoader implements
	...
	public UserEnrolment loadRecord (final int id)
		...
		try {
			...
			st = con.prepareStatement ("SELECT * FROM subscriptions WHERE id=?");
			st.setInt (1, id);
			rs = st.executeQuery ();
			rs.next ();
			return set (rs);
	...
	public UserEnrolment loadRecord (final String orderNumber)
			throws NotFoundException {
		final String orderSource = UserEnrolment
				.getOrderSource (orderNumber);
		final String orderCode = UserEnrolment
				.getOrderCode (orderNumber);
		Connection con = null;
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("SELECT * FROM subscriptions WHERE order_source=? AND order_code=?");
	...
	public void saveRecord (final UserEnrolment record) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("UPDATE subscriptions SET user_id=?, begins_at=?, expires_at=?, product_id=?, auth_sub_id=? WHERE order_source=? AND order_code=?");
