...
public class Enrolment extends SimpleDataRecord <Enrolment> {
	...
	public static Enrolment [] getAvailableEnrolments () {
		...
		try {
			...
			st = con
					.prepareStatement ("SELECT ID FROM products WHERE available='Y' AND visible='Y'");
			if (st.execute ()) {
				rs = st.getResultSet ();
				while (rs.next ()) {
					...
					try {
						enrolment = Nomenclator.getDataRecord (
								Enrolment.class, rs.getInt ("ID"));
					} catch (final NotFoundException e) {
						// Default catch action, report bug
						// (twheys@gmail.com, Oct
						// 30,
						// 2009)
	 ...
	 */
	@Deprecated
	public static Enrolment getByID (final int productID) throws NotFoundException {
	 ...
	 */
	@Deprecated
	public static Enrolment getByProductCode (final String productCode2) throws NotFoundException {
		return Nomenclator.getDataRecord (Enrolment.class, productCode2);
	}
	...
	/**
	 * the number of months for each renewal interval. Note that monthly
	 * renewals are <em>not</em> 30-day renewals; they will take into
	 * account things like “he subscribed on the 31° January, so the
	 * next renewal is on the 1° March because February is too short”
	 * and stuff like that. (Note, this example might be incorrect…)
	 */
...
// /**
	...
	public void setRenewalMonths (final int newRenewMonths) {
		renewMonths = newRenewMonths; changed();
	}
