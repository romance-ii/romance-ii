...
public class MBField_NumericString extends MBField_String {
	...
	public MBField_NumericString (final MBSession mySession,
			final MBFieldIdent myIdent, final int leastDigits, final int mostDigits) {
		super (mySession, myIdent);
		minDigits = leastDigits;maxDigits=mostDigits;
	}
	...
	protected boolean checkValue (final String newValue) {
		return super.checkValue (newValue)&& newValue.length () >= minDigits && newValue.length () <= maxDigits && !newValue.matches ("[^0-9]") ;
	}
