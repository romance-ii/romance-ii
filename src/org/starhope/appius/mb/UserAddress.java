...
public class UserAddress extends SQLPeerDatum {
	...
	/**
	 * The Internet Domain Name of the service used for validation
	 * (e.g. "google.com" for Google Maps); null, if the address has
	 * not been validated
	 */
	...
	/**
	 * Request an asynchronous validation against some online service
	 * or other (Google, Yahoo, whoever)
	 */
	public void asyncValidate () {
		UserAddress.log.error ("Address validation not implemented");
	}
	
	/**
	 * <p>
	 * Convenience method, to match up against the address fields as
	 * provided by Authorize.Net. Plus, it might come in useful in
	...
	public void set (final JSONObject o) {
		try {
			if (o.has ("user")) {
				/*
				 * try { setUser (User.get (o.getJSONObject
				 * ("user"))); } catch (final Exception e) {
			...
			UserAddress.log.error ("Exception", e);
		}
	}
