...
public enum MBErrorReason {
	 ...
	 */
	BLANK ("In order to continue, please fill in the required field %s"),
	/**
	 * The confirmation of a field failed; for eMail or password entries
	 * that have to be entered twice
	 */
	CONFIRM (
	...
	public String getErrorMessage (final MBFieldIdent field) {
		String fieldEnum = field.toString ().toLowerCase (
				Locale.ENGLISH);
		return LibMisc.getTextOrDefault ("mb." + fieldEnum + "."
				+ toString ().toLowerCase (Locale.ENGLISH), String
				.format (defaultMessageFormat, field.getName ()));
	}
}
