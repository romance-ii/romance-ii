...
public class RoomPlace {
	...
	public RoomPlace makePlace (final RoomPlaceType placeType,
			final String placeName, final GeneralPath placeShape,
			final String... placeAttributes) throws GameLogicException {
		switch (placeType) {
			case evt_$:
				return new ServerEventPlace (placeName,placeShape,placeAttributes);
			case evt_item_:
		...
		case evt__:
			...
			case walk:
				return new WalkSpace (placeName, placeShape,
						placeAttributes);
			case obs_:
				return new ObstacleSpace (placeName,placeShape,placeAttributes);
			case xpt_:
				return new TransportSpace (placeName, placeShape,
						placeAttributes);
		}
		throw new GameLogicException ("unknown place type", placeType, placeName);
	}
