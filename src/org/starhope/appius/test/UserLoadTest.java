...
/**
 * WRITEME: The documentation for this type (UserLoadTest) is
 * incomplete. (brpocock@star-hope.org, Mar 15, 2010)
 * 
 * @author ewinkelman@resinteractive.com Ed Winkelman
 * @author twheys@gmail.com Timothy W. Heys
 * @author ewinkelman@resinteractive.com Ed Winkelman
...
public class UserLoadTest extends ServerThread {
	...
	/**
	 * WRITEME: document this field (brpocock@star-hope.org, Mar 15,
	 * 2010) serial (UserLoadTest)
	 */
	...
	public void run () {
		...
		// log.debug ("Instantiating user for load test, #"
		// + (10000 + serial));
		final AbstractUser myGuy = Nomenclator
				.getUserByID (10000 + serial);
		if (null == myGuy) {
			// log.debug ("No such user.");
			return;
		}
		// log.debug ("Log In processing");
		...
		while (true) {
			...
			// log.debug ("Speech for load test");
			myGuy.speak ("I want you to know / that I'm happy for you / I wish nothing but / the best for you both...");
		}
	}
	 ...
	 * This is an overriding method.
	 * 
	 * @see org.starhope.appius.net.ServerThread#sendLoginPacket(java.lang.String,
	...
	/**
	 * WRITEME: document this method (brpocock@star-hope.org, Mar 15,
	 * 2010)
	 */
	private void spawnChildren () {
		for (int n = 0; n < AppiusConfig.getIntOrDefault (
				"org.starhope.appius.test.loadUsers", 2000); ++n) {
			// log.debug ("Instantiating UserLoadTest/" + n);
			Zone z = AppiusClaudiusCaecus.getAllZones ().get (0);
			ZoneSpawner.checkZonesForSpawn ();
			z = null;
			while (null == z) {
				final LinkedList <Zone> zones = AppiusClaudiusCaecus
						.getAllZones ();
				final int zNum = AppiusConfig.getRandomInt (0,
						zones.size () - 1);
				// log.debug ("Looking for a zone: let's try #" +
				// zNum);
				z = zones.get (zNum);
			...
			// log.debug ("Choosing a zone for load test " + n
			// + ", got “" + z.getName () + "“");
			final UserLoadTest t = new UserLoadTest (n, z);
			// log.debug ("Starting UserLoadTest/" + n);
			t.start ();
			try {
				Thread.sleep (100);
			} catch (final InterruptedException e) {
				BugReporter
						.getReporter ("srv")
						.reportBug (
								"Caught a InterruptedException in run",
								e);
			}
		}
		// log.debug ("Done creating load users.");
		return;
	}
