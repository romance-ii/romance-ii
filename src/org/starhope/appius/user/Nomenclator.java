...
public class Nomenclator {
	...
	/**
	 * Store an object into a caché in which the object is identified
	 * by a DataRecord instance object. For this method to function,
	 * the object must implement the {@link DataRecordBacked}
	 * interface. The representative class of the object must also be
	 * specified, allowing “comparable” implementations of a common
	 * base class to be stored in the same caché.
	 * 
	 ...
	 *             being stored is either a member of, or a member of
	 *             an extending/implementing class thereof.
	 ...
	 *             identifiers of this DataRecord will be used as keys
	 *             to the caché.
	 */
	public static <RecordClass extends DataRecord, StorageBaseClass extends DataRecordBacked <RecordClass>, InstanceClass extends StorageBaseClass> void cache (
		...
		final long cacheTime = AppiusConfig
				.getIntOrDefault (klass.getCanonicalName ()
						+ ".cacheTime", 5 * 60000);
	...
	/**
	 * Store an item into a caché, for which we've already extracted
	 * the object's identification string and integer values.
	 * 
	...
	public static <T extends DataRecord> void cache (
		...
		if (null == o.getRecordLoader ()) {
			Nomenclator.log.error (
					" Warning: caching record with no loader: []",
					o.toString (), new RuntimeException ());
		}
	...
	public static <T extends Object> void cache (final T o,
		...
		try {
			...
			Nomenclator.log
					.error ("Caught a CacheException in Nomenclator.cache ",
							e);
			return;
		}
		cache.store (ident, o);
	...
	public static AbstractUser create (final Date date,
		...
		try {
			result = AppiusConfig
					.getUserClass ()
					.getConstructor (Date.class, String.class,
							String.class)
					.newInstance (date, characterClass, login);
		} catch (final IllegalArgumentException e) {
			Nomenclator.log.error (
					"Caught a IllegalArgumentException in create",
					e);
			...
			Nomenclator.log
					.error ("Caught a InstantiationException in create",
							e);
		} catch (final IllegalAccessException e) {
			Nomenclator.log
					.error ("Caught a IllegalAccessException in create",
							e);
			...
			Nomenclator.log
					.error ("Caught a InvocationTargetException in create",
							e);
	...
	public static <T extends DataRecord> T findInCache (
		...
		final Cache <Integer, T> cache = Nomenclator
				.getIDCache (klass);
		T o = null;
		...
		try {
			...
			Nomenclator.log.error ("Incorrect type stored in cache",
					e);
			o = null;
		}
		if (null == o) {
	 ...
	 * @param klass The class of which the data record is an instance
	 *             or instance of a subclass thereof
	 ...
	 * @param ident The string identifier for the object to be found
	 *             (or not)
	 ...
	 * have objects of a totally incorrect class returned as a result
	 * of a search if the identifier is not globally unique. This
	 * method provides a minimum guarantee of correctness by catching
	 * class cast exceptions and throwing a somewhat more
	 * useful/informative {@link NotFoundException} rather than forcing
	 * the caller to perform a blind cast and try to catch runtime
	 * exceptions.
	 * 
	 ...
	 *              (parameter) “ident” which is not of the class
	 *              “klass” or a subclass or implementor thereof.
	 */
	@SuppressWarnings ("unchecked")
	public static <T extends Object> T findInGenericCache (
		...
		try {
			...
			Nomenclator.log
					.error ("Caught a CacheException in Nomenclator.findInCache ",
							e1);
	...
	public static AbstractUser get (final JSONObject object) {
		if (object.has ("id")) {
			try {
				return Nomenclator.getUserByID (object
						.getInt ("id"));
	 ...
	 * </p>
	 * <p>
	 * The ID value given is expected to be globally unique for all
	 * data records of the given class. Note that this is most often an
	 * SQL-backed {@link DataRecord}, and the ID is generally an SQL
	 * row identification. Also, typically, these ID's are expected to
	 * be positive numbers. However, this method will cheerfully accept
	...
	public static <T extends DataRecord> T getDataRecord (
		...
		try {
			...
			final T rec = AppiusConfig.getRecordLoaderForClass (
					klass).loadRecord (id);
			if (null == rec) {
	...
	@SuppressWarnings ("unchecked")
	public static <T extends DataRecord> T getDataRecord (
			final String klass, final int id)
	...
	public static <T extends DataRecord> T getDataRecord (
		...
		return Nomenclator.getDataRecord (
				(Class <T>) Class.forName (klass), ident);
	}
	 ...
	 *             (which must accept the nomen gentile of the
	 *             parameter classes)
	...
	public static <T extends Iterator <? extends DataRecord>> T getDataRecordIterator (
		...
		for (int i = 0; i < constructorArgs.length; ++i) {
			argTypes [i] = Nomenclator
					.getNomenGentile (constructorArgs [i]
							.getClass ());
		}
		...
		try {
			return (T) Class
					.forName (AppiusConfig.getConfigOrDefault (
							"xtn." + klass.getCanonicalName ()
									+ ".factory",
							klass.getCanonicalName ()
									+ "DefaultFactory"))
					.getMethod ("iterator", argTypes)
					.invoke (null, constructorArgs);
		} catch (final IllegalArgumentException e) {
			Nomenclator.log
					.error ("Caught a IllegalArgumentException in Nomenclator.getDataRecordIterator ",
							e);
		} catch (final SecurityException e) {
			Nomenclator.log
					.error ("Caught a SecurityException in Nomenclator.getDataRecordIterator ",
							e);
		} catch (final IllegalAccessException e) {
			Nomenclator.log
					.error ("Caught a IllegalAccessException in Nomenclator.getDataRecordIterator ",
							e);
		} catch (final InvocationTargetException e) {
			Nomenclator.log
					.error ("Caught a InvocationTargetException in Nomenclator.getDataRecordIterator ",
							e);
		} catch (final NoSuchMethodException e) {
			...
			Nomenclator.log
					.error ("Caught a NoSuchMethodException in Nomenclator.getDataRecordIterator (ctor for "
							+ typeBuilder.toString (), e);
		} catch (final ClassNotFoundException e) {
			Nomenclator.log
					.error ("Caught a ClassNotFoundException in Nomenclator.getDataRecordIterator ",
							e);
		}
		return result;
	}
	...
	/**
	 * get the caché for ID (integer) records of a given class (or
	 * gens)
	 * 
	...
	public static int getIDForLiveUserName (final String name) {
		try {
			return Nomenclator.findInCache (
					AbstractUser.class,
					Nomenclator.findInGenericCache (
							UserRecord.class, name))
					.getUserID ();
		} catch (final NotFoundException e) {
			return -1;
		}
	...
	private static AbstractUser getInstanceNPCByName (
		...
		if (midDollar > 1) {
			final String instanceOf = instanceName.substring (0,
					midDollar).toLowerCase (Locale.ENGLISH);
			try {
				Integer.parseInt (instanceName
						.substring (midDollar + 1));
			} catch (final NumberFormatException e) {
				Nomenclator.log.error (
						"Instanced NPC format error “"
								+ instanceName + "”", e);
				return null;
			}
			
			for (final WeakReference <AbstractUser> possible : Nomenclator
					.getOnlineNPCs (instanceOf)) {
				final String wantLabel = instanceName
						.toLowerCase (Locale.ENGLISH);
				final AbstractUser u = possible.get ();
				if (null != u) {
	...
	public static String getLoginForID (final int id) {
		...
		try {
			...
			st = con.prepareStatement ("SELECT userName FROM users WHERE ID=?");
	...
	public static Set <WeakReference <AbstractUser>> getOnlineNPCs (
		...
		final Set <WeakReference <AbstractUser>> npcs = Nomenclator.npcMap
	...
	/**
	 * get a record ID for something that is in turn backed by some
	 * kind of data record
	 * 
	...
	private static <T, S> Cache <S, T> getSomeCache (
		...
		try {
			...
			if (e.getMessage ().contains (klass.getCanonicalName ())) {
				someCache = Nomenclator.initializeClassCache (
						sClass, klass, s);
			} else {
				Nomenclator.log
						.error ("Caught a CacheException in Nomenclator.getSomeCache ",
								e);
				return null;
			}
		}
	...
	public static AbstractUser getUserByID (final int id) {
		try {
			...
			Nomenclator.log
					.error ("Caught a NotFoundException in Nomenclator.getUserByID ",
							e);
			return null;
		}
	}
	...
	public static AbstractUser getUserByLogin (final String login) {
		final AbstractUser u = Nomenclator
	...
	public static AbstractUser getUserByRequestedName (
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("SELECT ID FROM users WHERE requestedName=?");
	...
	public static int getUserIDForLogin (final String name) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("SELECT ID FROM users WHERE userName=?");
	...
	public static AbstractUser [] getUsersByMail (final String mail) {
		...
		try {
			...
			Nomenclator.log.error ("SQLException", e);
		} finally {
	...
	private static <S, T> Cache <S, T> initializeClassCache (
		...
		try {
			...
			throw new RuntimeException (
					"Caught a CacheException in cache creation ",
					e1);
		}
	}
	 ...
	 * @param record The record to be used to instantiate the new
	 *             object
	...
	public static <T extends DataRecordBacked <S>, S extends DataRecord> T instantiate (
		...
		try {
			...
			throw new RuntimeException (
					"Extension class mismatch, cannot use "
							+ xtnClassName + " for " + klassName,
					e);
		} catch (final ClassNotFoundException e) {
			Nomenclator.log
					.error ("ClassNotFoundException trying to get "
							+ xtnClassName + " for " + klassName,
							e);
		}
		...
		try {
			constructor = subclass.getConstructor (record
					.getClass ());
		} catch (final SecurityException e) {
			throw new RuntimeException (
		...
		if (null == constructor) {
			throw new RuntimeException ("Can't find <init>("
	...
	public static boolean isLoginForbidden (
		...
		if ( !User.isNameValid (userNameRequested)) {
			Nomenclator.log.warn ("Name is invalid "
					+ userNameRequested);
			return true;
		}
		...
		if (1 < userNameNoNum.length ()) {
			final FilterResult carlSays = AppiusConfig.getFilter (
					FilterType.USER_LOGIN).filterMessage (
					userNameNoNum);
			...
			if ( (carlSays.status == FilterStatus.Black)
				...
				if (AppiusConfig
					...
					Nomenclator.log.warn (new Date (System
							.currentTimeMillis ())
							+ " ---- "
							+ userNameRequested
							+ "("
	...
	public static <S extends DataRecord, T extends S> void killCache (
		...
		try {
			...
			try {
				...
				Nomenclator.log
						.error ("Unhandled NotFoundException in killCache",
								e);
				return;
			}
	...
	public static <T extends DataRecord> T make (
		...
		try {
			...
			try {
				return klass.getConstructor (parmTypes)
						.newInstance (more);
			} catch (final Exception e1) {
				Nomenclator.log
						.error ("in Nomenclator.make (NoSuchMethod/sub) …",
								e1);
				throw new NotReadyException (e1);
			}
		} catch (final Exception e) {
			Nomenclator.log.error ("in Nomenclator.make…", e);
	...
	public static <T extends SQLPeerDatum> T makeSQLPeer (
			...
			throws NotReadyException {
		Nomenclator.log
				.warn ("WARNING: Using deprecated makeSQLPeer for "
						+ klass.getCanonicalName ()
						+ "; update to DataRecord, please!");
		try {
			return klass.getConstructor (Object [].class)
					.newInstance (more);
		} catch (final NoSuchMethodException e) {
			final Class <?> parmTypes[] = new Class <?> [more.length];
			for (int i = 0; i < more.length; i++ ) {
	...
	public static synchronized AbstractUser noteNPC (
		...
		final WeakReference <AbstractUser> ref = new WeakReference <AbstractUser> (
				whom);
		...
		npcs.add (ref);
		Nomenclator.npcMap
				.put (userRecord.getLogin ().toLowerCase (
						Locale.ENGLISH), npcs);
		return whom;
	}
	...
	public static void removeInstanceNPC (final GeneralUser npc) {
		final Set <WeakReference <AbstractUser>> set = Nomenclator.npcMap
				.get (AbstractNonPlayerCharacter
						.getNameStripped (npc.getAvatarLabel ()
								.toLowerCase (Locale.ENGLISH)));
		if (null != set) {
			for (final WeakReference <AbstractUser> ref : set) {
				final AbstractUser who = ref.get ();
				if (who.equals (npc)) {
					set.remove (ref);
				}
			}
