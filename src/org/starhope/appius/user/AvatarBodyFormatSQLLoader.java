...
public class AvatarBodyFormatSQLLoader implements
	...
	public AvatarBodyFormat loadRecord (final int id)
		...
		try {
			...
			st = con
					.prepareStatement ("SELECT * FROM avatarBodyFormats WHERE id=?");
			st.setInt (1, id);
			rs = st.executeQuery ();
			rs.next ();
			AvatarBodyFormat body = new AvatarBodyFormat (this);
			return load (body, rs);
	...
	public AvatarBodyFormat loadRecord (final String identifier)
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("SELECT * FROM avatarBodyFormats WHERE name=?");
	...
	public void removeRecord (final AvatarBodyFormat record) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con
					.prepareStatement ("DELETE FROM avatarBodyFormats WHERE id=?");
			st.setInt (1, record.getID ());
			rs = st.executeQuery ();
		} catch (SQLException e) {
			AppiusClaudiusCaecus
	...
	public void saveRecord (final AvatarBodyFormat record) {
		...
		AppiusClaudiusCaecus.reportBug("unimplemented RecordLoader<AvatarBodyFormat>::saveRecord (brpocock@star-hope.org, Sep 21, 2010)");
