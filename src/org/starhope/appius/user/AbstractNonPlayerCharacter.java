 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
	...
	private final class RealSpeakRunner implements Runnable {
		...
		RealSpeakRunner (final AbstractNonPlayerCharacter anNPC,
				final String newSpeech, final Channel newChannel) {
			npc = anNPC;
			speech = newSpeech;
			channel = newChannel;
		}
	 ...
	 * @throws NotFoundException if the user login given doesn't yield
	 *              a valid data record
	...
	protected AbstractNonPlayerCharacter (final UserRecord dataRecord)
			throws NotFoundException, GameLogicException {
		super (dataRecord);
		if (AgeBracket.System != getUserRecord ().getAgeGroup ()) {
			throw new GameLogicException ("not NPC", dataRecord,
					this);
		}
	...
	/**
	 * Get all rooms currently accessible from the present room, via
	 * normal doors.
	 * 
	 * @return the set of all rooms potentially accessible
	 */
	protected Set <String> getAccessibleRooms () {
		final Set <String> accessible = new ConcurrentSkipListSet <String> ();
		accessible.add (getRoom ().getFullMoniker ());
		final Set <String> seen = new ConcurrentSkipListSet <String> ();
		while ( !accessible.containsAll (seen)) {
			for (final String moniker : accessible) {
				...
				for (final Pair <String, Polygon> exit : r
						.getExits ().values ()) {
	...
	public String getAvatarLabel () {
		return getUserRecord ().getLogin () + "$" + instanceID;
	}
	...
	public JSONObject getPublicInfo_new () {
		...
		try {
			...
			if (avatarClass.canColor ()) {
				colors.put ("0", userRecord.getBaseColor ()
						.toInt ());
				colors.put ("1", userRecord.getExtraColor ()
						.toInt ());
				colors.put (
						"2",
						User.getOutlineColourForBaseColour (
								userRecord.getBaseColor ())
								.toInt ());
			}
	...
	public void inviteBuddy (final AbstractUser newBuddy) {
		...
		try {
			request.put ("sign", UserList.getBuddyApprovalCookie (
					this, newBuddy));
	...
	public void liftBan (final AbstractUser authority)
		...
		authority.acceptMessage ("Lift Ban", getUserRecord ()
				.getLogin (),
	...
	public void speakCasually (final String text) {
		...
		if (LibMisc.hasText (key)) {
			...
			casualSpeechQueue.add (LibMisc.getText (key
					+ "."
					+ AppiusConfig.getRandomInt (0, Integer
							.parseInt (LibMisc.getText (key
									+ ".d")) - 1)));
		} else {
			AppiusClaudiusCaecus
	...
	public void tick (final long currentTime, final long deltaTime)
		...
		if ( (lastSpoken < (currentTime - casualSpeechRate))
			...
			speak (casualSpeechQueue.remove ());
	...
	/**
	 * Perform some action when the current movement-target position
	 * has been reached
	 * 
	 * @param runnable what to do when the target position is (or at
	 *             least, should have been) reached
	 * @return the handle to the event, in case it's needed to cancel
	 *         it
