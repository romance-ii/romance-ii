...
Map <Integer, Room> {
    ...
    /**
     * The view of the house from outside is hacked in like this for now
     * XXX
     */
        ...
        try {
            ...
            if (st.execute () && null != (rs = st.getResultSet ())) {
                while (rs.next ()) {
                    ...
                    hasRooms.put (Integer.valueOf(i), Boolean.TRUE);
                }
            }
    ...
    private void addRooms (final Integer [] integers) {
        ...
        try {
            ...
            st =
                con
                .prepareStatement ("INSERT IGNORE INTO userRooms (userID, roomID) VALUES (?,?)");
            ...
            AppiusClaudiusCaecus.reportBug ("Caught a SQLException in addRoom", e);
        } finally {
            LibMisc.closeAll (rs,st,con);
        }
    }
    ...
    private void instantiateRoom (final Integer key) {
        if (Boolean.FALSE.equals (hasRooms.get(key) )) {
			try {
				houseRooms.put (key, Room.initUserRoom (owner, key
						.intValue ()));
    ...
    public void putAll (final Map <? extends Integer, ? extends Room> m) {
        ...
        if (! (nn instanceof Integer [])) {
            AppiusClaudiusCaecus.reportBug ("odd");
            return;
        }
        addRooms ((Integer []) nn);
        houseRooms.putAll (m);
