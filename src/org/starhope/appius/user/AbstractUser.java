...
Comparable <Object>, HasVariables, Serializable,
	 ...
	 * @param label A label which nominally identifies the source of
	 *             the message
	 */
	...
	/**
	 * Removes a special effect from a user
	 * 
	 * @param sfx WRITEME 
	 */
	 ...
	 * to be distinct as user names are. (At any given moment, there
	 * can be only one object with a given avatar label in the room,
	 * but there can be only one user with a given name in the
	 * multiverse.) For users, this is the user name.
	 * 
	...
	/**
	 * Gets the user's public information
	 * 
	 * @param s The channel listener requesting the information
	 * @return The datagram representing the user's public info
	 */
	public ADPUserInfo getDatagram (ChannelListener s);
	 ...
	 * 
	 * @return A string with the user's login string and user ID
	 *         number, formatted for debugging output
	 */
	...
	/**
	 * Height in pixels. This is used as a scaling-factor for the
	 * avatar file; it's assumed that other dimensions of the avatar
	 ...
	 * @return WRITEME 
	 */
	ADPUserSFX getUserSFXDatagram (ChannelListener s);
	 ...
	 * @return the coördinates in the current room toward which the
	 *         user should proceed
	 */
	 ...
	 * 
	 * @return true, if this user needs a parent account associated
	 *         with it per COPPA
	 */
	 ...
	 *              register/associate new child accounts (usually due
	 *              to having other child accounts which are banned)
	 * @throws GameLogicException if this user account does not need a
	 *              parent record (adult account or system/robot
	 *              account)
	...
	/**
	 * set the time at which the player started moving — not
	 * necessarily the same as {@link #getTravelStart()} because that
	 * value is updated over time, this value is when they actually
	 * started moving along the path altogether
	 * 
	...
	/**
	 * Makes the user speak into the current default channel of the
	 * room the user is in
	 * 
	 * @param string the speech (or emote)
	 */
