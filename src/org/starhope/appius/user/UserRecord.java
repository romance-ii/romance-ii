...
public class UserRecord extends SimpleDataRecord <UserRecord> implements
	...
	/**
	 * The age bracket to which this user belongs: currently kid
	 * (0-12), teen (13-17), or adult (18+). Computed from birth date
	 ...
	 * which the account was approved (by parent) or eMail was
	 * validated (by self). Either way, it's basically eMail
	 * validation, but just differs in who gets the mail.
	 */
	...
	/**
	 * the dialect of the user's {@link #language} in which s/he
	 * prefers to see messages
	 */
	private String dialect = "US";
	
	/**
	 * For kids, this is the date on which eMail Plus secondary eMail
	 * is sent out. This may be past or future.
	 */
	private Date emailPlusDate = null;
	
	/**
	 * The extra colour of the avatar. For Master Toot (Amphibious)
	 * this is the nose/highlight colour
	 */
	...
	/**
	 * Whether or not changes to the class will be flushed. True: Do
	 * not flush False: Flush
	 */
	...
	/**
	 * the time at which the user last was logged on
	 */
	private Timestamp lastActive = new Timestamp (0);
	...
	/**
	 * the date on which the user's mail address was confirmed (or
	 ...
	 * may have a system-assigned random one), so we need to nag them
	 * to pick a name.
	 */
	 ...
	 */
	final private SecurityCapability sysOpCapability = SQLPeerEnum
			.get (SecurityCapability.class,
					SecurityCapability.CAP_SYSOP_COMMANDS
							.intValue ());
	/**
	 * The set of rooms in this user's house (and yard). Special
	 * values: 0 is the first room (everyone gets it for free), and 1
	 * is the yard (lot).
	 */
	...
	public UserRecord () {
		...
		mail = null;
		try {
			avatarClass = Nomenclator.getDataRecord (
					AvatarClass.class, 1);
	 ...
	 */
	public UserRecord (final Date birthDate1,
			final String avatarTitle, final String userNameRequest,
		...
		// Give initial avatar name in string format, ex. 'zap',
		// 'human'
		try {
			avatarClass = Nomenclator.getDataRecord (
					AvatarClass.class, avatarTitle);
		} catch (final NotFoundException e) {
			throw AppiusClaudiusCaecus
					.fatalBug (
							"Caught a NotFoundException in UserRecord.UserRecord ",
	...
	public UserRecord copyProtoype (final UserRecord prototype) {
		...
		setLanguage (prototype.getLanguage (),
				prototype.getDialect ());
		...
		setPasswordRecovery (
				prototype.getPasswordRecoveryQuestion (),
	 ...
	 * form for debugging. The form is: “u:LOGIN=#ID” where LOGIN is
	 * the user's login name, and ID is their numeric
	 * 
	 ...
	 * 
	 * @return the user ID of the moderator who kicked this user
	 *         offline (1 for the system)
	 */
	...
	/**
	 * @param whether if true, the parents have approved the account;
	 *             if false, they did not (cancel the account)
	 */
	public void parentApprovedAccount (final boolean whether) {
		if ( (null != approvedDate)
				|| (AgeBracket.Kid != setAgeGroup ())) {
			return;
		}
		if (1 > parentID) {
			return;
	...
	public void requestNewUserName (final String userNameRequested)
		...
		try {
			...
			BugReporter.getReporter ("srv").reportBug (
					"God can't auto-approve user "
							+ userNameRequested + " name?", e);
		}
		/*
	 ...
	 */
	public void setLastActive (final Timestamp lastActive) {
	 ...
	 */
	public void setParentApprovedName (
			final boolean parentApprovedName1) {
