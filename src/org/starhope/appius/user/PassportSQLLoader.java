...
public class PassportSQLLoader implements PassportLoader {
	...
	throws NotFoundException
	{
		...
		passport.setRecordLoader (this);
		passport
		.setOwnerID (Nomenclator.getUserIDForLogin (identifier));
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st =
				con
				.prepareStatement ("SELECT moniker FROM userWorlds LEFT JOIN roomList ON worldID=roomList.ID LEFT JOIN users ON userID=users.ID WHERE userName=?");
	...
	public void removeRecord (final Passport record) {
		...
		AppiusClaudiusCaecus.reportBug("unimplemented RecordLoader<Passport>::removeRecord (brpocock@star-hope.org, Jul 22, 2010)");
	...
	public void saveRecord (final Passport record) {
		...
		try {
			...
			st =
				con
				.prepareStatement ("INSERT IGNORE INTO userWorlds (userID, worldID) SELECT ?, ID FROM roomList WHERE moniker=?");
