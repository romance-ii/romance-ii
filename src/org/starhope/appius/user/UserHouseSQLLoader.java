...
public class UserHouseSQLLoader implements UserHouseRecordLoader {
	 ...
	 */
	@Override
	public UserHouse loadRecord (final int id) throws NotFoundException {
		Connection con = null;
	...
	public void removeRecord (final UserHouse record) {
		...
		AppiusClaudiusCaecus.reportBug("unimplemented RecordLoader<UserHouse>::removeRecord (brpocock@star-hope.org, Jul 22, 2010)");
	 ...
	 */
	private void setRooms (final UserHouse record, final PreparedStatement getRooms) {
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next ()) {
				...
				try {
					record.put (Integer.valueOf (roomID), Room
							.initUserRoom (owner, roomID));
