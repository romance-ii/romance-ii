 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
	 ...
	 */
	public List <String> findPathsTo (final String from,
			final String to) {
	...
	public String getRoomToward (final String room) {
		...
		for (final String other : user.getZone ().getRoomMap ()
				.get (thisRoom)) {
	...
	public boolean pathTo (final Coord3D target) {
		...
		if ( !currentRoom.canWalk (target)) {
			...
			currentRoom.goTo (user, currentRoom
					.goTo_clipToWalkSpace (user, target), null,
					"Walk");
			return true;
		}
	...
	public boolean seekRoom (final Room r) {
		...
		final String nextRoom = getRoomToward (r.getMoniker ());
		if (null != nextRoom) {
			try {
				if (false == pathTo (Geometry.getRandomPointWithin (
						currentRoom.getExitTo (nextRoom))
						.toCoord3D ())) {
	...
	public synchronized void takei (final Coord3D target) {
		...
		if ( !pathTo (target)) {
			timedEvent = whenAtTarget (new ContinuePathRunner (
					target));
		}
	}
	...
	public long whenAtTarget (final Runnable runnable) {
		return kalendor.schedule (
				Geometry.getTimeToTarget (user,
