 ...
 * @author brpocock@star-hope.org Bruce-Robert Pocock
 */
public class UserIgnoreListSQLIterator extends UserListSQLIterator {
	 ...
	 * @param user WRITEME
	 */
	public UserIgnoreListSQLIterator (final AbstractUser user) {
		super (user, "$ignore");
	}
	...
	/**
	 * @param ignoreID user ID to ignore
	 */
	public void add (final int ignoreID) {
		Connection con = null;
		java.sql.PreparedStatement st = null;
		final ResultSet rs = null;
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("INSERT IGNORE INTO ignoreList (userID, buddyID) VALUES (?,?)");
			st.setInt (1, userID);
			st.setInt (2, ignoreID);
			st.executeUpdate ();
		} catch (final SQLException e) {
			BugReporter.getReporter ("srv").reportBug (
					"Caught a SQLException in UserIgnoreListSQLIterator.add ("
							+ userID + "," + ignoreID + ")", e);
		} finally {
	...
	protected String getSQL () {
		return "SELECT userName AS otherName FROM ignoreList LEFT JOIN users ON users.ID=ignoreList.buddyID WHERE userID=?";
	}
	...
	public void remove (final AbstractUser interestingFellow) {
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("DELETE FROM ignoreList  WHERE userID=? AND buddyID = ?");
			st.setInt (1, userID);
			st.setInt (2, buddyID);
			st.executeUpdate ();
		} catch (final SQLException e) {
			BugReporter.getReporter ("srv").reportBug (
					"Caught a SQLException in UserIgnoreListSQLIterator.remove ("
							+ userID + "," + buddyID + ")", e);
		} finally {
			LibMisc.closeAll (rs, st, con);
		}
