...
public class EventRecord extends SimpleDataRecord <EventRecord>
	...
	public void cancel () {
		...
		try {
			...
			BugReporter
					.getReporter ("srv")
					.reportBug (
							"Caught a JSONException in EventRecord.cancel ",
							e);
		}
		return;
	}
	...
	private EventRecord copyFor (final AbstractUser player) {
		final EventRecord copy = new EventRecord (myLoader);
	...
	public EventRecord copyProtoype (final EventRecord prototype) {
		...
		id = prototype.id;
		medals.clear ();
		for (final MedalType medal : prototype.medals) {
			medals.add (medal);
		}
	...
	private BigDecimal determineRewardScalar (
		...
		if (outcome.isRewardRandom ()) {
			return BigDecimal.valueOf (AppiusConfig.getRandomInt (
					outcome.getRewardMin ().intValueExact (),
					outcome.getRewardMax ().intValueExact ()));
		}
	...
	public void end (final GenericItemReference itemPurchased,
		...
		Quaestor.getDefault ().action (
				new Action (getCreator (), "event.end.purchase",
						(AbstractUser) null, getEventType ()
								.getDescription (),
						itemPurchased, this));
		doOutcome ();
	 ...
	 * @throws GameLogicException if the transaction cannot be
	 *              completed
	 */
	public void end (final InventoryItem item, final AbstractUser u)
		...
		Nomenclator.getUserByID (creatorID).getInventory ()
				.add (item);
	 ...
	 * 
	 * @param sortedScores scores earned by all players of a
	 *             multiplayer event.
	 */
	public void end (
			final LinkedHashMap <AbstractUser, BigInteger> sortedScores) {
		final JSONObject multiplayerScores = new JSONObject ();
		int i = 0;
		for (final Map.Entry <AbstractUser, BigInteger> playerScore : sortedScores
				.entrySet ()) {
			final AbstractUser player = playerScore.getKey ();
			final BigInteger score = playerScore.getValue ();
			final JSONObject playerScoreJSON = new JSONObject ();
			try {
			...
			final EventRecord playerCopy = copyFor (player);
			playerCopy.end (score);
		}
		final JSONObject endEventInfo = new JSONObject ();
		try {
		...
		for (final AbstractUser player : sortedScores.keySet ()) {
		...
		try {
			...
			while (scores.next ()) {
				...
				if (scorePoints.compareTo (BigDecimal.ZERO) > 0) {
					...
					scoreInfo.put ("points",
							scorePoints.doubleValue ());
					scoreInfo.put ("ID", thatID);
					scoreInfo.put ("userName",
							scores.getString ("userName"));
					highScoreList.put (String.valueOf (i++ ),
							scoreInfo);
				}
			}
		} finally {
	...
	protected void putEventHighScoresIntoJSON (final JSONObject wrapper) {
		...
		try {
			...
			st = con.prepareStatement ("SELECT events.ID AS ID, points, userName FROM events LEFT JOIN users ON users.ID = events.creatorID WHERE eventTypeID = ? AND completionTimestamp > DATE(NOW() - INTERVAL 1 MONTH) ORDER BY points DESC LIMIT 24");
	...
	private void rewardWithCurrency (final EventOutcomeRecord outcome,
		...
		if ( (null != currencyEarned)
				...
				&& (BigDecimal.ZERO
						.compareTo (currencyAmountEarned) != 0)) {
			return;
		}
		...
		if (null != currencyUnits) {
			...
			setCurrencyEarned (currencyUnits.getCode (),
					currencyFigure);
	...
	private void rewardWithItem (final EventOutcomeRecord outcome,
		...
		switch (outcome.getRewardInventoryItemType ()) {
		case ITEM:
			try {
			...
			break;
		case COLLECTION:
			rewardItem = rewardWithItemFromCollection (outcome,
					scalar);
			break;
		case GLOBAL:
			rewardItem = GenericItemReference.getRandomItem (
					outcome.getMinRarity (),
					outcome.getMaxRarity ());
			break;
		default:
			return;
		}
		...
		if (null != rewardItem) {
			...
			final AbstractUser creator = getCreator ();
			final boolean hasItem = creator.getInventory ()
					.contains (Integer.valueOf (itemID));
			if ( !hasItem
					|| (hasItem && outcome
							.isPermitDuplicateReward ())) {
				setItemEarned (itemID);
				final InventoryItem rewarded = creator
	...
	private void rewardWithMedal (final EventOutcomeRecord outcome,
		...
		if ( (null != medal) && (scalar.signum () > 0)) {
			addMedalEarned (medal.getID ());
			Quaestor.getDefault ().action (
					new Action (getCreator (), "earned.medal",
							medal.getString ()));
		}
	}
	...
	/**
	 * @deprecated use {@link EventType#setHighScores(boolean)}
	 */
	@Deprecated
	public void setSendHighScore (final boolean whether) {
		// no op
	}
	...
	public JSONObject toJSON () throws JSONException {
		...
		if (0 < creatorID) {
			jso.put ("creator",
					Nomenclator.getLoginForID (creatorID));
		}
		if ( (null != currencyAmountEarned)
				&& (null != currencyEarned)) {
			...
			if (AppiusConfig
				...
				curr.put ("amt",
						currencyAmountEarned.round (
								MathContext.DECIMAL32)
								.intValue ());
			} else {
				curr.put ("amt",
						currencyAmountEarned.toPlainString ());
			}
			...
			if (AppiusConfig
				...
				if (currencyEarned.equals (Currency.getPeanuts ())) {
					jso.put ("peanuts", currencyAmountEarned
							.round (MathContext.DECIMAL32)
							.intValue ());
					jso.put ("totalPeanuts",
							getCreator ().getWallet ().get (
									Currency.getPeanuts ()));
					// TODO
