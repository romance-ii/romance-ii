...
public class MedalRecordSQLLoader extends MedalRecordLoader {
	...
	public void changed (final MedalRecord changedRecord) {
		...
		BugReporter.getReporter ("srv").reportBug (
				"Unimplemented: MedalRecordSQLLoader.changed called with "
						+ changedRecord.toString ());
	}
	 ...
	 */
	@Override
	public MedalRecord loadRecord (final int id)
		...
		try {
			...
			st = con.prepareStatement ("SELECT * FROM medalTypes WHERE ID=?");
			st.setInt (1, id);
			rs = st.executeQuery ();
	...
	public MedalRecord loadRecord (final String identifier)
		...
		try {
			con = AppiusConfig.getDatabaseConnection ();
			st = con.prepareStatement ("SELECT * FROM medalTypes WHERE name=?");
	...
	public void removeRecord (final MedalRecord record) {
		...
		BugReporter
				.getReporter ("srv")
				.reportBug (
						"unimplemented RecordLoader<MedalRecord>::removeRecord (brpocock@star-hope.org, Jul 22, 2010)");
	...
	public void saveRecord (final MedalRecord record) {
		// XXX
		BugReporter.getReporter ("srv").reportBug (
				"Unimplemented: MedalRecordSQLLoader.saveRecord called with "
						+ record.toString ());
	}
