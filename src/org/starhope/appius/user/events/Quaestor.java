...
public class Quaestor extends TimerTask {
    ...
    public static void action (final String verb,
            final String indirectObject) {
        Quaestor
        .dispatchAction (null, null, verb, null, indirectObject);
    }
    ...
    public static int getEndedEventCount (final AbstractUser user,
        ...
        try {
            ...
            st = con
            .prepareStatement ("SELECT COUNT(*) FROM events WHERE eventTypeID = ? AND creatorID = ? and completionTimestamp IS NOT NULL");
            ...
            AppiusClaudiusCaecus.reportBug (
                    "Caught an Exception in getEndedEventCount", e);
        } finally {
    ...
    public static int getEventCount (final AbstractUser user,
        ...
        try {
            con = AppiusConfig.getDatabaseConnection ();
            st = con
            .prepareStatement ("SELECT COUNT(*) FROM events WHERE eventTypeID = ? AND creatorID = ?");
            st.setInt (1, eventTypeID);
            st.setInt (2, user.getUserID ());
            final ResultSet rs = st.executeQuery ();
            if (rs.next ()) {
    ...
    throws NotFoundException {
        return Nomenclator.getDataRecord (EventType.class, eventTypeID);
    }
     ...
     * @return Number of Occurrences
     */
    public static int getItemGainedEventCount (final AbstractUser user,
            final String moniker, final int itemID) {
        int result = 0;
        PreparedStatement st = null;
        Connection con = null;
        int eventTypeID = 0;
        ...
        try {
            con = AppiusConfig.getDatabaseConnection ();
            st = con
            .prepareStatement ("SELECT COUNT(*) FROM events WHERE eventTypeID = ? AND creatorID = ? AND itemGained = ?");
            st.setInt (1, eventTypeID);
            st.setInt (2, user.getUserID ());
            st.setInt (3, itemID);
            final ResultSet rs = st.executeQuery ();
    ...
    /**
     * Gets a list of all events of the given type that were started but
     * not yet ended
     *
    ...
    public static List <Integer> getStartedEventsByType (
        ...
        try {
            con = AppiusConfig.getDatabaseConnection ();
            st = con
            .prepareStatement ("SELECT ID FROM events WHERE eventTypeID = ? AND creatorID = ? AND completionTimestamp IS NULL");
            st.setInt (1, eventTypeID);
            st.setInt (2, user.getUserID ());
            final ResultSet rs = st.executeQuery ();
            while (rs.next ()) {
    ...
    public static void listen (final ActionHandlerInterface handler) {
        System.out
        .println (" Quaestor.listen | " + handler.toString ());
	...
	/**
	 * Set the default Quaestor to be used when {@link #getDefault()} is
     ...
     */
    public void tryAction (final Action action) throws NotFoundException
    {
        if (AppiusConfig
                .getConfigBoolOrFalse ("org.starhope.appius.jdbc.useTomcat")) {
            final String via = AppiusConfig
