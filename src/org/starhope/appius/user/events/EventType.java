...
public class EventType extends SimpleDataRecord <EventType> implements
	...
	private List <EventRecord> getPriorForPlayer (
		...
		try {
			...
			st = con.prepareStatement ("SELECT ID FROM events WHERE creatorID=? AND creationTimestamp > NOW() - INTERVAL ? MINUTE AND eventTypeID=?");
	 ...
	 *              already at (or more than) the limit for a given
	 *              time period. The general response to the user
	 *              should reflect something to the effect of “this
	 *              event cannot be performed again, now” or suppress
	 *              notification of the event altogether.
	 */
	...
	public EventRecord startEvent (final AbstractUser user)
		...
		Quaestor.getDefault ().action (
				new Action (user.getRoom (), user, "event.start",
						(AbstractUser) null, getString (), ev));
		return ev;
	}
	...
	public JSONObject toJSON () {
		...
		try {
			...
			BugReporter.getReporter ("srv").reportBug (
					"Caught a JSONException in EventType.toJSON ",
					e);
		}
		return o;
	}
