...
public class EventOutcomeRecordSQLLoader implements
		...
		try {
			...
			if ("R".equals (onDupe)) {
				...
				throw AppiusClaudiusCaecus
						.fatalBug ("onDupe invalid as “"
								+ onDupe
								+ "” for event outcome with ID "
								+ id);
			}
			rec.setRewardByPoints ("Y".equals (rs
					.getString ("rewardPoints")));
			rec.setRewardCollectionID (rs
					.getInt ("rewardCollectionID"));
			...
			final String rewardType = rs
					.getString ("rewardInventoryItemType");
	...
	@Override
	public void refresh (final EventOutcomeRecord record) {
	...
	public void removeRecord (final EventOutcomeRecord record) {
		...
		BugReporter
				.getReporter ("srv")
				.reportBug (
						"unimplemented RecordLoader<EventOutcomeRecord>::removeRecord (brpocock@star-hope.org, Jul 22, 2010)");
