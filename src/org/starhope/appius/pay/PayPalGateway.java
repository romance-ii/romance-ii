...
public class PayPalGateway implements PaymentGatewayReal {
	...
	public PayPalGateway () {
		 ...
		 * environment, and ensure that only authorized individuals
		...
		try {
			...
			profile.setAPIPassword (AppiusConfig.getConfigOrDefault (
					"com.paypal.sdk.apiPassword",
					"QFZCWN5HZM8VBG7Q"));
			profile.setSignature (AppiusConfig
					.getConfigOrDefault (
	...
	public void startEnrolment (final Payment payment)
		...
		try {
			...
			BugReporter.getReporter ("srv").fatalBug (e1);
		}
		/*
	...
	public void transactPayment (final Payment payment)
		...
		final UserAddress billingAddress = payment.getCredentials ()
				.getAddress ();
		encoder.add ("STREET", billingAddress.getAddress () + " "
				+ billingAddress.getAddress2 ());
		encoder.add ("CITY", billingAddress.getCity ());
		encoder.add ("STATE", billingAddress.getProvince ());
		encoder.add ("ZIP", billingAddress.getPostalCode ());
		encoder.add ("COUNTRYCODE", billingAddress.getCountry ());
