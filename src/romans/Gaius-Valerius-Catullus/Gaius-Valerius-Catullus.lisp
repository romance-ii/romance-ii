...
after processing."
  (let ((subject (normalise-noun noun))
        (object (normalise-noun object))
        (predicate (if (stringp predicate) (make-keyword (cffi:translate-camelcase-name predicate))))))
...
(defmethod utterance-formatting-string (predicate (language-code (eql :en)))
  (string-case predicate
    ...
    ("/r/dbpedia/knownFor" "~0@*~a is known for ~2@*~a.")
    ("/r/dbpedia/languageFamily" "~0@*~a is a language in the family of languages which is ~2@*~a.")
    ("/r/dbpedia/languageFamily" "~0@*~a is a language which is spoken in the location ~2@*~a.")
    ("/r/dbpedia/influencedBy" "~0@*~a was influenced in their work by ~2@*~a")
    ("/r/dbpedia/mainInterest" "~0@*~a is mainly interested in ~2@*~a")
