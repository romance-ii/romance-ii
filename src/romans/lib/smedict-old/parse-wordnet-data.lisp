...
;;; Fixed bug (?) in parsing index-file-entry
;;; Dan G. Tecuci, April 2000
...
(defun parse-index-file-entry (entry)
  "Given a string as returned by INDEX-ENTRY-FOR-WORD, decode it and
return the elements of the index entry.

returns: (values word part-of-speech poly_cnt pointer-types
synset-offsets)"
  ...
  (when entry
	...
	;; Increased nr of synset offsets to skip an extra int
	;; that appear before the real list (DGT 2000)
	(dotimes (i (+ (parse-integer (next-token)) 1)
