...
;;; CommonLisp interface to WordNet; © 1995, Mark Nahabedian,
;;; Artificial Intelligence Laboratory, Massachusetts Institute of
;;; Technology

;;; Code for reading WordNet "data" and "index" files as output by
;;; WordNet's "grind" command.  These functions merely return the
;;; requested entry as a string. These are other functions which will
;;; parse those strings and extract the data.

;;; This file provides the functions INDEX-ENTRY-FOR-WORD and
;;; READ-DATA-FILE-ENTRY which are used to read raw data from
;;; WordNet's compiled database files. You must tell them which file
;;; to read from using the FILE-DESCRIPTION argument.  You can pass a
;;; file stream (as opened by WITH-WORDNET-FILE-STREAM), a pathname or
;;; a part of speech keyword as the value for this argument.

;;;  (INDEX-ENTRY-FOR-WORD file-description word) Looks up word in the
;;; appropriate index file and returns the string corresponding to
;;; that record of the index file.

;;; (READ-DATA-FILE-ENTRY file-description offset) Reads a WordNet
;;; "symset" record from the specified offset in the specified file. A
;;; string is returned. Offset was either read from an index record,
;;; or from a pointer description in another synset record.


;;; Modified April 2000 to work with Allegro Common Lisp on Solaris
;;; Dan G. Tecuci, Univ of Texas at Austin

(defpackage wn (:use :cl)
            (:export #:))

(in-package wn)

(defparameter +wordnet-database-directory+
  #+(and naha Genera) #p"mu:/usr/local/wordnet/dict/"
  #+naha #-Genera #p"n:/naha/WordNet/princeton/dict/"
  #+proj #p"/projects/wordnet/wordnet-1.6/dict/"
  #+tecuci #p"/u/tecuci/cs395/wordnet1.5/dict"
  (make-pathname :host "wordnet" :directory "wn"))
...
(defmacro with-wordnet-file-stream ((stream-var file-identifier) &body body)
  `(with-open-file (,stream-var ,file-identifier
                                :direction :input
                                ;; we need to do the following so we
                                ;; don't get a fucking epsilon stream
                                ;; that we can't seek on.
                                :element-type
