...
(defparameter +wordnet-database-directory+
                                        ;#+Genera #p"mu:/usr/local/wordnet/dict/"
                                        ;#-Genera #p"n:/naha/WordNet/princeton/dict/"
  (merge-pathnames "WordNet-3.0/dict/" (translate-logical-pathname #p"R2SRC:"))
                                        ;#p "/projects/wordnet/wordnet-1.6/dict/"
                                        ;#p "/u/tecuci/cs395/wordnet1.5/dict"
  )
    ...
    (loop
       ...
       (cond ((null raw-entry)
             ...
             ((and (>= where top)
              ...
              (return nil))
             ((>= where top)
              ;; The line that TRY points into is the last line of the
              ;; current search space.  Start searching from BOTTOM.
              (setq try bottom))
